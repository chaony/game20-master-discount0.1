local M = class("SkinShopControl", LikeOO.OOControlBase)
...
function M:buyForSDK(charge_id)
    if self.m_model.m_is_token == true then
        self:buyUseVoucher(
            charge_id,
            function()
                self:updateMsg("refresh_ui", {ui_name = "Summer.SkinShop"}, "Summer.SummerMain")
            end
        )
        return
    end
    ...
    if cfg then
        local function rechargeBack(params)
            ...
            if self.m_model then
                self.m_model.m_params.skin_bought[1] = 1
                self:updateMsg("refresh_ui", {ui_name = "Summer.SkinShop"}, "Summer.SummerMain")
            end
        end
