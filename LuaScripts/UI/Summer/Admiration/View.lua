local M = class("AdmirationView", LikeOO.OOPopBase)

M.m_size_type = 2
M.m_uiName = "Summer/Admiration"
...
function M:setMoney()
    ...
    local price_old_text = GameUtil:getMoneyTypeNum(price_old)-- Language:getTextByKey("gf_str_0029", )
    self:setTextByLanKey("price_Number", price_old_text)
    --self:setTextByLanKey("buyprice",price_new .. "元购买" .. "new_str_0037")
    local price_new_text = GameUtil:getMoneyTypeNum(price_new)
    self:setTextByLanKey("buyprice", price_new_text)
    self:setTextByLanKey("per_text", return_per)

    local hero_type = string.split(Language:getTextByKey(check_item.des), "·")
...
function M:refreshUI()
    ...
    if self:isSkinShow() then
        if self.m_model.m_params.skin_bought[1] ~= nil then --已经购买过英雄
            get_btn_show = true
        else --没购买过英雄
            btn_buy_show = true
        end
