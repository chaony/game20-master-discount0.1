local M = class("LuckyCharmControl",LikeOO.OOControlBase)
...
--支付
function M:buyForSDK(date)
    local charge_id = date.cell_data.cfg.charge_id
    if self.m_model.m_is_token == true then
        self:buyUseVoucher(charge_id, function ()
            self:updateMsg("refresh_ui", { ui_name = "Summer.LuckyCharm" }, "Summer.SummerMain")
