local M = class("LuckyDogDrawPopControl", LikeOO.OOControlBase)
...
function M:onHandle(msg , data)
	...
	elseif msg == "add_btn" then
		...
		if self.m_model.m_buyNum + 1 > self.m_model.m_max_num then
			GameUtil:lookInfoTips(self, {msg = Language:getTextByKey("lucky_dog_014"), delay_close = 2})
			return
		end
		self.m_model.m_buyNum = self.m_model.m_buyNum + 1;
	elseif msg == "reduce_btn" then
		if self.m_model.m_buyNum <= 1 then
			GameUtil:lookInfoTips(self, {msg = Language:getTextByKey("lucky_dog_015"), delay_close = 2})
			return
		end
		self.m_model.m_buyNum = self.m_model.m_buyNum - 1;
	elseif msg == "max_btn" then
		...
		if self.m_model.m_buyNum + 10 > self.m_model.m_max_num then
			GameUtil:lookInfoTips(self, {msg = Language:getTextByKey("lucky_dog_014"), delay_close = 2})
			return
		end
