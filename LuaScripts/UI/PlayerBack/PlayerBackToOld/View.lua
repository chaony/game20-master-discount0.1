local M = class("PlayerBackToOldView",LikeOO.OOPopBase)

M.m_uiName = "PlayerBack/PlayerBackToOld"
M.m_size_type = 2
...
function M:updateLoopScrollCell(index, cell_object, cell_data)
    ...
    if index == self.selected_index then
        ...
        LuaBehaviourUtil.setText(luaBehaviour, "day_display_text", self.m_model:getDayWordContent(index))
        ...
        LuaBehaviourUtil.setText(luaBehaviour,"content_text", self.m_model:getDayWordContent(index))
         ...
         if index < self.selected_index then
            ...
            --左边已领取
             if reward_status == 2 then
                 ...
                 LuaBehaviourUtil.setText(luaBehaviour,"content_text", "已领取")
            --左边未领取
             else
