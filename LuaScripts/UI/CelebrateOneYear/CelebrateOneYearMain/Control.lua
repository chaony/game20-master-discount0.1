local M = class("CelebrateOneYearMainControl",LikeOO.OOControlBase)
...
function M:onHandle(msg , data)
    ...
    elseif msg == "explain_btn" then
        ...
        self:openView("Pops.CommonHelpPop", {title = title_name, content = "tid#AnniversaryEvent"})
    else
...
function M:tryOpenItem(btn_name)
    ...
    local isShow,show_time = self:isActiveFuture(item)
    if isShow == false then
        GameUtil:lookInfoTips(self, {msg = show_time, delay_close = 2})
        return
    end
    if self.m_model:getItemTimeLimit(btn_name) == 0 then
        GameUtil:lookInfoTips(self, {msg = Language:getTextByKey("celebrate_one_year_text_0009"), delay_close = 2})
        --GameUtil:lookInfoTips(self, {msg = Language:getTextByKey("celebrate_one_year_text_0006"), delay_close = 2})
        return
    end
    if item then
        ...
        local params = {
            ...
            refresh_main = "CelebrateOneYear.CelebrateOneYearMain",
            refresh_name = "CelebrateOneYear.CelebrateOneYearMain"}
        if item.open_id == 422 then --试炼展示击破
        ...
        elseif item.open_id == 426 then
            params.items_table = {
                ...
                {open_id = 422, btn_name = "item_2_btn", prefab_folder = "Chivalry",prefab_name = "ChivalryBattle",show_name = false,is_close_view = true}, 	--神都机关令
...
function M:isActiveFuture(item) --判断活动是将要开启 并返回字符
    local cur_tim = UserDataManager:getServerTime() --服务器时间
    local end_ts = item.show_active_data.end_time
    local start_ts = item.show_active_data.start_time
