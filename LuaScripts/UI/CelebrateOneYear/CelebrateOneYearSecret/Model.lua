local M = class("CelebrateOneYearModel", LikeOO.OODataBase)
...
function M:onEnter()
    ...
    self.m_open_id = self.m_params.open_id or 369
...
function M:getMaxFloor()
    ...
    if not scroll_tab then
        Logger.logError("coolSummerSecret not get cfg monster_reward")
        return 
    end
...
function M:getCostNum()
    ...
    if not scroll_tab then 
        Logger.logError("coolSummerSecret not get cfg monster")
        return
    end
...
function M:getScrollCfg()
    ...
    local version = self.m_data and self.m_data.version or 0
    local cfg = scroll_tab[version]
    return cfg
end
