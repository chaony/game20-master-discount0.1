local M = class("CelebrateOneYearControl", LikeOO.OOControlBase)
...
function M:onHandle(msg , data)
    if msg == 99999 then -- 关闭
		...
		self:closeView()
    elseif msg == "open_scroll" then --点击奖励列表
        self:openScroll(data)
    elseif msg == "get_ace_pag_btn" then --道具不足，确定按钮
        --QuickOpenFuncUtil:openFunc(100001, { open_id = 368, level_up = 1 })
        --self:closeView()
    elseif msg == "yulan_btn" then  --更换奖励  
        local params = {
    ...
    elseif msg == "reward_btn" then
        ...
        self:openView("DoubleFestivalActivity.SmashEggBuyPop", {m_openId = self.m_model.m_open_id, m_version = self.m_model.m_data.version, is_token = self.m_model.m_is_token,callback = callback})
