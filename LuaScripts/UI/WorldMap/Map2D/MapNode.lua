...
M.m_uiName = "Map/MapNode"
M.m_iphoneXAdapter = true

local OPEN_BTN_TAB = {
	{ btn_key = "bag_btn", open_id = 90, lock = true}, --行囊
	{ btn_key = "qy_btn", open_id = 91, lock = true}, --奇遇
	{ btn_key = "completion_reward_btn", open_id = 92, lock = true}, --传奇
	{ btn_key = "travels_btn", open_id = 92, lock = true}, --游记
}
...
function M:onEnter()
	...
	--进入的场景
	local open_map = UserDataManager.local_data:getUserDataByKey("big_map_enter_map", {})
	
	if open_map[tostring(self.map.map_id)] ~= 1 then
		self:findNewArea()
		open_map[tostring(self.map.map_id)] = 1
	end
	UserDataManager.local_data:setUserDataByKey("big_map_enter_map", open_map)
...
function M:refreshPathNode()
	...
	if scene_lines_data ~= nil then
		local area = scene_lines_data[tostring(SceneManager.curScene.sceneId)]
		if area ~= nil then
	...
	if self.route_obj == nil then
		self.route_obj = ResourceUtil:LoadUIGameObject("Map/MapRoute/Route"..self.map.motherMapId, Vector3.zero, path_mask)
	end
	if self.route_obj ~= nil then
...
function M:onButtonClick(obj, name)
	...
	elseif name == "close_btn" then
		self.m_control:updateMsg("guide_close_mapNode")
		LikeOO.Map2DControl:backToHigherMap()
		--返回挂机
	elseif name == "guide_btn" then
		self.m_control:updateMsg("guide_close_mapNode")
		--self.m_control:updateMsg(99999)
		LikeOO.Map2DControl:closeMap(false)
			--帮助界面
	...
	elseif name == "path_btn" then
		...
		--聊天
	elseif name == "chat_btn" then
		self.m_control:openView("Chat2")
		--奇遇
	elseif name == "qy_btn" then
		self.m_control:openView("WorldMap.WorldMapTask.WorldMapEvents", {task_id = 0})
		--行囊
	...
	-- 威望
	elseif name == "prestige_btn" then
		self.m_control:openView("WorldMap.WorldMapPrestige")
	elseif name == "bag_btn_mask" then --行囊遮罩
		local red_flag ,tips_str =  BtnOpenUtil:isBtnOpen(90)
		GameUtil:lookInfoTips(self.m_control, { msg = tips_str, delay_close = 2})
	elseif name == "qy_btn_mask" then --奇遇遮罩
		local red_flag ,tips_str =  BtnOpenUtil:isBtnOpen(91)
		GameUtil:lookInfoTips(self.m_control, { msg = tips_str, delay_close = 2})
	elseif name == "completion_reward_btn_mask" then --完成度遮罩
		local red_flag ,tips_str =  BtnOpenUtil:isBtnOpen(92)
		GameUtil:lookInfoTips(self.m_control, { msg = tips_str, delay_close = 2})
	elseif name == "prestige_btn_mask" then --威望遮罩
	...
	elseif name == "box_btn" then
		...
		if status == 1 then
			...
			local params = {area_id = SceneManager.curScene.sceneId, scene_id = self.map.motherMapId}
...
function M:onButtonClickPathBtn()
	...
	local params = {}
	params.area_id = SceneManager.curScene.sceneId
	params.scene_id = self.map.motherMapId
	params.mode = 2
	self:openView("WorldMap.WorldMapTaskReward", params)
end
...
function M:dataUpdateEvent(event, data)
	...
	if  curEvent == "task_done_update" then
		self:refreshRedPoint()
		self:refreshPathNode()
	end
end
...
function M:getMapCpd()
	...
	if regional_task_done then
		local area = regional_task_done[tostring(SceneManager.curScene.sceneId)]
		if area ~= nil then
		...
		local params = {
			...
			click_func = function(index, cell_object, cell_data, click_object, click_name)
				...
				if cell_data.status == 5 then
					...
					--local targetMapId = self.map:getMotherMapId(task.chapter)
					--if targetMapId == self.map.motherMapId then
					--	self.m_control:openView("WorldMap.WorldMapTask.WorldMapEvents")
					--
					--	--GameUtil:lookInfoTips(self.m_control, {msg = Language:getTextByKey("new_str_0582"), delay_close = 2})
					--else
					--	local targetMap = self.map_table[targetMapId]
...
function M:refreshTaskInfo()
	...
	if show_task ~= nil then
		...
		luaBehaviour:RegistButtonClick(function (obj, name)
			--self.m_control:openView("WorldMap.WorldMapTask.WorldMapEvents", {open_tab_index = 1, task_id = show_task})
			local params = { area_id = SceneManager.curScene.sceneId}
			self:openView("WorldMap.WorldMapLegend", params)
		end)
