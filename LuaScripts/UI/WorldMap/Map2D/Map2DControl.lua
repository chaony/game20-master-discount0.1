...
function M:openMap2D( id , callback)
    self:changeScene(id, function()
        ...
        self.curMap2D = require("UI.WorldMap.Map2D.Map2D").new(self.m_control, {map_id = id, is_first = isFirst, callback = callback})
...
function M:back()
    if self.stack:hasData() then
        ...
        self:changeScene(cur_map_id, function()
            ...
            self.curMap2D = require("UI.WorldMap.Map2D.Map2D").new(self.m_control, {map_id = cur_map_id, is_first = false})
        end)
    else
        if self.curMap2D ~= nil then
            self:closeMap()
    ...
    if self.curMap2D ~= nil then
        ...
        if mother_map_id ~= self.curMap2D.map_id then
            self:changeScene(mother_map_id, function()
                ...
                self.curMap2D = require("UI.WorldMap.Map2D.Map2D").new(self.m_control, {map_id = mother_map_id, is_first = false})
            end)
        else
            self:closeMap()
        end
...
function M:changeScene(map_id, changeFunc)
    if self.changeSceneNode == nil then
        local tab_cls = CustomRequire("UI.WorldMap.Map2D.ChangeSceneNode")
        ...
        --self.changeSceneNode:setOrder(self.changeSceneNode.m_sortOrder + 50)
        --self.m_control:setOnceTimer(1, function()
        --    self.changeSceneNode:destroy()
        --    self.changeSceneNode = nil
        --end)
    end
end
