local M = class("PeakArenaFightModel", LikeOO.OODataBase)
...
function M:onEnter()
	...
	if self.m_battle_logs and next(self.m_battle_logs) == nil then
		self.is_promotion = true --轮空 直接晋级
	else
		self.is_promotion = false
	end
	self.m_selete_battle_log = 1 --队伍索引/分组索引
	self.m_max_teaams = #self.m_battle_logs or 3 --一共有几组小组赛	
	...
	self:checkIndex()
	self.m_rank_pos = self:getMyNewPos()
end

function M:checkIndex()
	for i,v in ipairs(self.m_player_data_32) do
		for kk,vv in pairs(v) do
			if vv.uid == self.m_id then
				self.m_battle_index_64 = i
				return
			end
		end
	end
...
function M:setRankProgress()
	if next(self.m_data.players) ~= nil then
		self.m_rank_progress = 2
	elseif next(self.m_data.group_data) ~= nil then
		self.m_rank_progress = 1
	else
		self.m_rank_progress = 0
	end
end
...
function M:getRankType()
	...
	elseif self.m_top_data.week == 2 then 
		...
		for i = min,max do
			local c_p_data = self.m_data.players[tostring(i)]
...
function M:checkMyGroup()
	if next(self.m_data.group_data) == nil then
		return false
	end
	for k,v in pairs(self.m_data.group_data.users) do
...
function M:checkMyGroupWinner()
	...
	if self.m_data.group_data.winner == self.m_id then
		return true
	end
end
...
function M:myBattleLog()
	if next(self.m_data.group_data) ~= nil and next(self.m_data.group_data.battle_log) ~= nil then
		local m_logs = {}
		for i = 1, #self.m_data.group_data.battle_log do
			local c_battle_log = self.m_data.group_data.battle_log[i]
...
function M:getMyEnemyId()
	...
	local min, max = self:getCurBattleStatus()
	for i = min,max do
		local c_data = self.m_data.players[tostring(i)]
	...
	if parent_pos*2 == m_pos then
		return self.m_data.players[tostring(parent_pos*2 +1)]
	else
		return self.m_data.players[tostring(parent_pos*2)]
	end
end

--我的最新位置
function M:getMyNewPos()
	for i = 1,64 do
		local c_data = self.m_data.players[tostring(i)]
		if c_data and c_data.uid == self.m_id then
			return i
...
function M:getUsersById(id)
	return self.m_data.group_data.users[tostring(id)]
end

function M:getUserHeadAvatar(avater)
	local cfg = ConfigManager:getPlayerPictureCfg(avater)
	if cfg == nil or next(cfg) == nil then
		local cur_skin_cfg = ConfigManager:getHeroSkinCfg(avater)
		if cur_skin_cfg then
			return cur_skin_cfg.hero_spine
		else
			return "hero_0181_SkeletonData"
		end
	end
	...
	if self.m_top_data.week == 1 then
		if #self.m_battle_logs > 0 then
			...
			if left == 1 then
				...
				local m_team = UserDataManager.hero_data:getMultTeamByKey("top_arena")
				if next(m_team) == nil then
...
function M:check8Win()
	...
	if self.m_data.players[tostring(8)] == nil then
		return false
	end 
	return true
end
...
function M:get64BattleLog()
	...
	for t_key = team_index, 8 do
		for i = 1,8 do
			...
			local c_player = self.m_data.players[tostring(index)]
			if c_player then
...
function M:get32BattleLog()
	...
	local new_tab = {}
	local team_index = 1
	for t_key = team_index, 4 do
		for i = 1,8 do
			local index = 32 + i - 1 + (t_key-1) * 8
			local c_player = self.m_data.players[tostring(index)]
			if c_player then
				c_player.index = index
				if new_tab[t_key] then
...
function M:get4BattleLog()
	...
	local team_index = 1
	for t_key = 4, 7 do
		local c_player = self.m_data.players[tostring(t_key)]
		if c_player then
...
function M:getPlayerDataByIndex4(index)
	local team_s = self.m_player_data_4[self.m_battle_index_64] or {}
	if next(team_s) ~= nil then
		return team_s
	end
	return nil
...
function M:getWinPlayerData2(index)
	local team_s = self.m_data.players[tostring(index)] or {}
	if next(team_s) ~= nil then
		team_s.index = index
		return team_s
	end
...
function M:getWinPlayerData3(index)
	local team_s = self.m_data.players[tostring(1)] or {}
	if next(team_s) ~= nil then
		team_s.index = index
		return team_s
	end
...
function M:getMyScoreRank()
	return self.m_data.score_rank or 0
end
