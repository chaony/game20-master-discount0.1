local M = class("LuckyRabbitGiftPopView",LikeOO.OOPopBase)

M.m_uiName = "SeasonPreview/SeasonGiftBag"
M.m_size_type = 2

function M:onEnter()
	self:setTextByLanKey("common_title_text", "lucky_rabbit_hut_main_007")
	self:refreshUI()
end
...
function M:updateLoopScroll()
	...
	local page_data = self.m_model:getGiftBagData() or {}
	local data = page_data[1] or {}
	...
	if self.m_loop_scroll_view == nil then
		...
		local params = {
			...
			click_func = function(index, cell_object, cell_data, click_object, click_name)
				...
				if status ~= -1 then
					self:updateMsg(click_name, {index = cell_data.place_index, cell_data = cell_data})
...
function M:updateScrollViewCell(index, cell_object, cell_data)
	...
	if refresh == 0 then -- 无限制
		...
		if refresh == 1 then
			LuaBehaviourUtil.setTextByLanKey(luaBehaviour,"limit_times_text", "new_str_1025", time_limit - cell_data.times)
		else
			LuaBehaviourUtil.setTextByLanKey(luaBehaviour,"limit_times_text", "new_str_0796", time_limit - cell_data.times)
		end
	end
	local sort = cfg.price_type or 1
	LuaBehaviourUtil.setObjectVisible(luaBehaviour, "cost_node", sort == 1 and status ~= 2)
	LuaBehaviourUtil.setObjectVisible(luaBehaviour, "buy_btn_text", sort == 2 and status ~= 2)
	if sort == 1 then -- 元宝
	...
	LuaBehaviourUtil.setObjectVisible(luaBehaviour, "buy_btn", status ~= 2)
	...
	task_finish_text.gameObject:SetActive(status == 2)
end
