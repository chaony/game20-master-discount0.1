...
function M:onEnter()
	...
	self.mission_tab = ConfigManager:getCfgByName("new_regional_delegation")
	local task_tab = ConfigManager:getCfgByName("new_regional_task")
	local task_team_tab = ConfigManager:getCfgByName("new_regional_task_team")
...
function M:getBanchTask()
	local data = {}
	local task_done = UserDataManager:getNewMapRegionalTaskDoneData()
	local task_team_tab = ConfigManager:getCfgByName("new_regional_task_team")
	local task_tab = ConfigManager:getCfgByName("new_regional_task")
...
function M:getSenceReward()
	local task_tab = ConfigManager:getCfgByName("new_regional_map")
...
function M:getMissionData()
	...
	local delegationData = UserDataManager:getNewMapDelegationData()
...
function M:getCurTaskMaps()
	...
	local task_team_tab = ConfigManager:getCfgByName("new_regional_task_team")
	local task_tab = ConfigManager:getCfgByName("new_regional_task")
	local article_tab = ConfigManager:getCfgByName("new_regional_article")
	local tasks = UserDataManager:getNewMapTasksData()
	for k,v in pairs(tasks) do
