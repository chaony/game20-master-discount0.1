...
M.m_uiName = "WorldMapNew/WorldMapTaskReward/TaskRewardBranchNode"
M.m_iphoneXAdapter = true

function M:onEnter()
    self.m_task_node = self:findGameObject("task_node")
    self.panel = self:findGameObject("panel")
    self.task_team_tab = ConfigManager:getCfgByName("new_regional_task_team")
    self.map_tab = ConfigManager:getCfgByName("new_regional_map")
    self:refreshUI()
end
