local M = class("WorldMemoryLegendView",LikeOO.OOPopBase)

M.m_uiName = "WorldMapNew/WorldMapLegend"
M.m_size_type = 2
...
function M:onEnter()
	...
	self:setTextByLanKey("title_text", "worldMap_str_001")
...
function M:refreshUI()
	self:setTextByLanKey("left_bg_text", self.m_model.map_area_table[self.m_model.cur_area_id].name_m)
...
function M:updateScrollViewCell(index, cell_object, cell_data)
	...
	local open_flag, pre_task_end, tips_str = self.m_model:checkMapOpen(cell_data.id)
	...
	if open then
		if cell_data.task_count >= cell_data.total_task_count then
			...
			local show_redPoint = pre_task_end and table.indexof(enter_world_memory_list, cell_data.id) == false
