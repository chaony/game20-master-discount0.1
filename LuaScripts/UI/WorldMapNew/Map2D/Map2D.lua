...
function M:ctor(control, params)
    ...
    self.m_uiName = "NewMap/Map".. self.map_id;
...
function M:onCreate()
    ...
    --互动物品表
    self.article_table = ConfigManager:getCfgByName("new_regional_article")
    ...
    if self.motherMapId == self.map_id then
        ...
        if table.indexof(enter_world_memory_list, self.motherMapId) == false then
            table.insert(enter_world_memory_list, self.motherMapId)
            UserDataManager:setTempData("enter_world_memory_list", enter_world_memory_list)
            static_rootControl:updateMsg("refresh_point", nil, "WorldMapNew.WorldMemoryMain")
        end
    end

    local tab_cls = CustomRequire("UI.WorldMapNew.Map2D.MapNode")
...
function M:callback(data)
    ...
    if self.eventCount > 0 then
        --local taskDownData = UserDataManager:getNewMapRegionalTaskDoneData()
        --
    ...
    --处理所有任务
    self.allTask = table.copy(UserDataManager:getNewMapTasksData())
...
function M:newArticle( new_article )
    ...
    if new_article ~= nil and #new_article > 0 then
        local article_data =UserDataManager.local_data:getUserDataByKey("world_memory_newArticle", {})
        ...
        UserDataManager.local_data:setUserDataByKey("world_memory_newArticle", article_data)
    end
end
...
function M:onButtonClick(obj, name)
    ...
    if index ~= nil and index > 0 then
        ...
        LikeOO.NewMap2DControl:openMap2D(jumpto_id)
    end
    
    local function netCallback(response)
        if response then
            self:callback(UserDataManager:getNewMapArticlesData())
                ...
                for k,v in ipairs(response.cur_done_task) do
                    ...
                    if task.type == 3 or task.type == 4 then
                        if task.ending_map ~= 0 then
                            LikeOO.NewMap2DControl:openMap2D(task.ending_map)
                        end
    ...
    if npcIndex ~= nil and npcIndex > 0 then
        ...
        if team_id ~= nil then
            local eventCallback = function(event_data)
                ...
                if task.type == 1 then
                    ...
                    if self.tasks[npc_id].status == 2 then
                        self.m_model:getNetData("new_big_map_finish_task", {map_id = self.map_id, task_id = art_id}, function(response)
                            if response ~= nil then
                                local response_data = UserDataManager:getNewMapTasksData()
                                ...
                                if task.ending_event == 1 then
                                    --GameUtil:lookInfoTips(self.m_control, {msg = "任务完成", delay_close = 2})
                                    if self.m_cur_node then
                    ...
                    if event_data ~= nil and event_data["index"] ~= nil and event_data["id"] ~= 0 then
                        self:optionFinish(art_id, event_data, function(choice_id)
                            self.m_model:getNetData("new_big_map_select_task", { map_id = self.map_id, task_id = art_id, choice_id = self.allTask[tostring(art_id)].choice[event_data.index]}, function(response)
                                netCallback(response)
                ...
                elseif task.type == 2 then
                    ...
                    if self.tasks[npc_id].status == 2 then
                        ...
                        UserDataManager:setTempData("new_regional_battle_map_id", self.map_id)
                        UserDataManager:setTempData("new_regional_battle_task_id", art_id)
                    ...
                    if event_data ~= nil and event_data["index"] ~= nil then
                        self.m_model:getNetData("new_big_map_select_task", { map_id = self.map_id, team_id = team_id, choice_id = self.tasks[npc_id]["choice"][event_data["index"]]}, netCallback,nil,true,GlobalConfig.POST)
                    end
                else
                    local task_data = self.tasks[npc_id]
                    self:refreshChoice(task_data)
            ...
            if self.tasks[npc_id]["status"] == 1 then
                self.m_model:getNetData("new_big_map_start_task", {map_id = self.map_id, npc_id = npc_id, team_id = team_id }, netCallback,nil,true,GlobalConfig.POST)
            end
        --开始互动npc对话
        else
            if task_list == nil then
                self:playPlot(npc_id, "npc_plot", function(event_data)
                    if event_data == nil then
                        self.m_model:getNetData("new_big_map_interact", {map_id = self.map_id, art_type = self.tasks[npc_id]["art_type"], article_id = npc_id, choice_id = nil}, netCallback,nil,true,GlobalConfig.POST)
                    else
                        self:optionFinish(npc_id, event_data, function(choice_id)
                            self.m_model:getNetData("new_big_map_interact", {map_id = self.map_id, art_type = self.tasks[npc_id]["art_type"], article_id = npc_id, choice_id = choice_id}, netCallback,nil,true,GlobalConfig.POST)
                        end)
                    end
                end)
            else
                self:playPlot(npc_id, "npc_task_plot", function(event_data)
                    if event_data ~= nil then
                        if event_data.id ~= 0 then
                            self:optionFinish(npc_id, event_data, function(choice_id)
                                self.m_model:getNetData("new_big_map_start_task", {map_id = self.map_id, npc_id = npc_id, team_id = event_data.id}, function(response)
                                    if response ~= nil then
                                        netCallback(response)
                                        self:lookTips()
                                        self.new_Article = nil
    ...
    if artIndex ~= nil and artIndex > 0 and self.m_cur_node and self.m_cur_node.loadingId == nil then
        ...
        self:interact(art_id, obj, function(choice_id)
            m_model:getNetData("new_big_map_interact", {map_id = self.map_id, art_type = self.article_table[art_id].resource_type, article_id = art_id, choice_id = choice_id}, 
                function(response) 
                    netCallback(response)
                    self:lookTips()
                    self.new_Article = nil
    ...
    if opt_data ~= nil then
        local finishCallback = function()
            ...
            if opt_data.animation_open == 1 then
                LikeOO.NewMap2DControl:cameraShake(2, 0.3)
            end
            if opt_data.map_id ~= 0 then
                LikeOO.NewMap2DControl:openMap2D(opt_data.map_id)
            end
            
            self:lookTips()
            self.new_Article = nil
            static_rootControl:updateMsg("check_guide", nil, "WorldMapNew.WorldMapMain")
        end
        local callBack = function()
            if opt_data.event_battle ~= 0 then
                ...
                UserDataManager:setTempData("new_regional_battle_map_id", self.map_id)
                UserDataManager:setTempData("new_regional_battle_option_id", event_data.id)
                if self.allTask[tostring(art_id)] ~= nil then
                    UserDataManager:setTempData("new_regional_battle_task_id", art_id)
                    UserDataManager:setTempData("new_regional_battle_choice_id", event_data.index)
                elseif self.tasks[art_id] ~= nil then
                    UserDataManager:setTempData("new_regional_battle_art_type", self.tasks[art_id].art_type)
                    UserDataManager:setTempData("new_regional_battle_article_id", art_id)
                    UserDataManager:setTempData("new_regional_battle_choice_id", event_data.id)
                elseif self.arts[art_id] ~= nil then
                    UserDataManager:setTempData("new_regional_battle_art_type", self.arts[art_id].art_type)
                    UserDataManager:setTempData("new_regional_battle_article_id", art_id)
                    UserDataManager:setTempData("new_regional_battle_choice_id", event_data.id)
                end
...
function M:playPlot(id, type, finishCallback)
    local event = require("Battle.Sce.WorldScene.WorldMemoryEvent").new()
    ...
    elseif type == "art_plot" then
        ...
        event_data.isArticle = true
    elseif type == "battle_plot" or type == "enter_map_plot_new" or type == "no_choise_plot" then
...
function M:getEventChoiceData(eventStoryId, options, languages, canChoice, hasExit)
    ...
    local closeOptionData = UserDataManager:getNewMapCloseOptionData()
...
function M:battleFinish(result, replay, response)
    if replay ~= true then
        ...
        local option_id = UserDataManager:getTempData("new_regional_battle_option_id")
        ...
        if event_id ~= 0 then
            self:playPlot(event_id, "battle_plot", function()
                if ending_map ~= 0 then
                    LikeOO.NewMap2DControl:openMap2D(ending_map)
                end
                if result == 1 then
...
function M:clearBattleData()
    UserDataManager:setTempData("new_regional_battle_map_id", nil)
    UserDataManager:setTempData("new_regional_battle_task_id", nil)
    UserDataManager:setTempData("new_regional_battle_art_type", nil)
    UserDataManager:setTempData("new_regional_battle_article_id", nil)
    UserDataManager:setTempData("new_regional_battle_choice_id", nil)
    UserDataManager:setTempData("new_regional_battle_option_id", nil)
end
...
function M:refreshUI()
    ...
    local function netCallback(response)
        if response then
            ...
            if firstMap then
                UserDataManager:setNewMapArticlesData(response["articles"])
            ...
            local world_memory_newArticle =  UserDataManager.local_data:getUserDataByKey("world_memory_newArticle",{})
            self:newArticle(world_memory_newArticle[tostring(self.map_id)])

            UserDataManager.local_data:setUserDataByKey("world_memory_newArticle",{})

            local enter_map_plot = UserDataManager.local_data:getUserDataByKey("enter_map_plot_new", {})
            for k,v in pairs(self.allTask) do
                local task_id = tonumber(k)
                ...
                if task.map_event ~= nil and #task.map_event > 0 then
                    for k1, v1 in ipairs(task.map_event) do
                        if v1[1] == self.map_id and enter_map_plot[task_id.."_"..self.map_id] == nil then
                            self:playPlot(v1[2], "enter_map_plot_new", nil)
                            enter_map_plot[task_id.."_"..self.map_id] = 1
                            UserDataManager.local_data:setUserDataByKey("enter_map_plot_new", enter_map_plot)
                            break
                        end
                    end
                end
    ...
    if firstMap then
        if self.map_id == self.motherMapId then
            self.m_model:getNetData("new_big_map_enter_scene", {map_id = self.map_id, map_pos = { 0,0 }}, netCallback,nil,true,GlobalConfig.POST)
        else
            local function callback(response)
                if response then
                    firstMap = false
                    UserDataManager:setNewMapArticlesData(response["articles"])
                    self.m_model:getNetData("new_big_map_enter_child_scene", {map_id = self.map_id}, netCallback,nil,true,GlobalConfig.POST)
                end
            end
            self.m_model:getNetData("new_big_map_enter_scene", {map_id = self.motherMapId, map_pos = { 0,0 }}, callback,nil,true,GlobalConfig.POST)
        end
    else
        self.m_model:getNetData("new_big_map_enter_child_scene", {map_id = self.map_id}, netCallback,nil,true,GlobalConfig.POST)
    end
...
function M:dataUpdateHandler(eventName, data)
    ...
    self.newTasks = {}
    if data.event == "new_tasks_update" then
...
function M:openStageFormation(task_id, stage_id)
    ...
    SceneManager.lastScene = SceneManager.curScene.sceneId;
    ...
    if skip_deploy == 1 then -- 需要跳过Formation
        ...
        local function battleNetCallback(response)
            self:openView("GamePanel", {data = response, mode = GlobalConfig.BATTLE_MODE.NEW_BIG_MAP})
        end
        local params = {}
        ...
        self.m_model:getNetData("new_big_map_regional_battle_start", params, battleNetCallback, nil,nil,GlobalConfig.POST)
    else
        if stage_cfg.prologue_event ~= nil and stage_cfg.prologue_event ~= 0 then
            local function callback()
                if self:hasStoryEffect(stage_cfg) then
                    self:openView("Formation.BeforeStory", {battle_id = stage_id, type = "new_regional_battle_start", mode = GlobalConfig.BATTLE_MODE.NEW_BIG_MAP})
                else
                --    self:openView("Loading.BattleLoading", {callfunc = function(open_flag)
                --        if open_flag == "open_view" then
                            self:openView("Formation", {battle_id = stage_id, type = "new_regional_battle_start", mode = GlobalConfig.BATTLE_MODE.NEW_BIG_MAP})
                --        end
                --    end })
                end
            end
            ...
            if self:hasStoryEffect(stage_cfg) then
                ...
                self:openView("Formation", {battle_id = stage_id, type = "new_regional_battle_start", big_world_cur_scene_id = cur_scene_id, mode = GlobalConfig.BATTLE_MODE.NEW_BIG_MAP})
            end
        end
    end
end
