---@class HangRewardModel:OODataBase
local M = class("HangRewardModel", LikeOO.OODataBase)
...
function M:initData(data)
	...
	self.m_tim =  UserDataManager:getServerTime() - self.m_data.idle_start_time
	...
	self.max_second = self.m_data.idle_end_time - self.m_data.idle_start_time
end

function M:getTime()
	return GameUtil:formatTimeBySecond(math.min(self.max_second, (UserDataManager:getServerTime() - self.m_data.idle_start_time)), 999)
end
...
function M:getNum()
	...
	local jewel_hand_times = UserDataManager.jewel_data:getHandTimes() --秘宝额外增加挂机次数
	local all_num = table.copy(vip_cfg.quick_idle_times) + free_num + subscr_tim + jewel_hand_times --加上免费次数
