---@class HangRewardControl:OOControlBase
local M = class("HangRewardControl",LikeOO.OOControlBase)
    ...
    if #self.m_model.reward >0 then
        local function callfunc(data)
            if data then
                ...
                RewardUtil:rewardTipsByData(data.reward,nil, call_addmoney, {delay = 0, double = false, fly = true})
    	...
    	self.m_model:getNetData("get_idle_reward",nil, callfunc, nil, true)
    else
        self:closeView()
    end
end
