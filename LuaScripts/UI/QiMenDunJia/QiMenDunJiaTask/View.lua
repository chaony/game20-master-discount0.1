local M = class("QiMenDunJiaTaskView",LikeOO.OOPopBase)

M.m_uiName = "QiMenDunJia/QiMenDunJiaTask"
M.m_size_type = 2
...
function M:onEnter()
	self:setTextByLanKey("common_title_text", "qi_men_dun_jia_str_009")
	self:setTextByLanKey("auto_get_btn_text", "qi_men_dun_jia_str_047") --一键领取
	self:setTextByLanKey("explore_score_text", "qi_men_dun_jia_str_059")
	self:setTextByLanKey("reward_preview_btn_text", "qi_men_dun_jia_str_061")
	
	self.m_task_type_cache = {self.m_model.m_task_tag_mine, self.m_model.m_task_tag_union}
	self.m_loopscroll_view_cache = {}	--任务scrollview缓存
	self.m_loopscroll_reward_view_cache = {[self.m_task_type_cache[1]] = {}, [self.m_task_type_cache[2]] = {},}	--任务scrollviewcell中奖励scrollview的缓存
	
	self.m_toggle_btns = {}
	for k,v in pairs(__TAB_BTN_NODE) do
		self:setTextByLanKey(v.text_key, v.show_text)
		self.m_toggle_btns[k] = self:findToggle(v.btn_key)
		UIUtil.addToggleListener(self.m_toggle_btns[k], function(is_on) self:switchTabUpdate(is_on, k) end,nil,self.m_uiName)
		if k == self.m_model.m_select_tab_index then
			self.m_toggle_btns[k].isOn = true
...
function M:updateLoopScroll(scroll_view_key)
	local data = self.m_model:getTaskData(scroll_view_key)
	if self.m_loopscroll_view_cache[scroll_view_key] == nil then
		local loopscroll = self:findGameObject(scroll_view_key)
		local params = {
			...
			update_cell = function(index, cell_object, cell_data)
				self:updateLoopScrollCell(scroll_view_key, index, cell_object, cell_data)
			end,
		...
		self.m_loopscroll_view_cache[scroll_view_key] = LoopScrollViewUtil.new(params)
	else
		self.m_loopscroll_view_cache[scroll_view_key]:reloadData(data)
	end
end

function M:updateLoopScrollCell(scroll_view_key, index, cell_object, cell_data)
	local transform = cell_object.transform
	local luaBehaviour = UIUtil.findLuaBehaviour(transform)
	local node = luaBehaviour:FindGameObject("node")

	-- 名字
	UIUtil.setTextByLanKey(node.transform, "name_text", cell_data.cfg.name or "")
