local M = class("HuntTreasuresGuildMyTeamPop",LikeOO.OOControlBase)
...
function M:onEnter()
    ...
    self.m_timer2_id = self:setTimer(1, handler(self, self.updateTime2))
end
...
function M:onHandle(msg , data)
    ...
    elseif msg == "formation_btn" or msg == "formation_edit_btn" then
        local races = GameUtil:getActiveRacesByRegionId(1, self.m_model.m_version)
        self:openView("Formation",{mode = GlobalConfig.BATTLE_MODE.ACTIVE_MINING_DEFENSE, formation_index = data.index, races = races})
    ...
    elseif msg == "go_to_btn" then
        ...
        if oid > 0 then
            ...
            self:updateMsg("change_area", { region_id = region_id, location_id = location_id, close_view_name = "HuntTreasuresGuildMyTeamPop"  }, "HuntTreasuresGuild")
        end
    elseif msg == "go_to_btn2" then
        local index = data.index
        self:requestLocationIndex(index)
    elseif msg == "zhanbao_btn" then
        self:openView("HuntTreasuresGuild.HuntTreasuresGuildLogPop",{version = self.m_model.m_version})
    elseif msg == "wanguo_btn" then
        self:openView("HuntTreasuresGuild.HuntTreasuresGuildTXServer",{version = self.m_model.m_version})
    elseif msg == "bangdan_btn" then
        self:openView("HuntTreasuresGuild.HuntTreasuresGuildRank",{version = self.m_model.m_version})
    end
end
...
function M:requestSign(m_msg)
    ...
    local params = {}
    params.desc = desc
    params.ver = self.m_model.m_version
    self.m_model:getNetData("active_mining_set_desc", params, signCallback, nil, true)
end
...
function M:requestGetReward(team_id)
    ...
    self.m_model:getNetData("active_mining_receive_income", { team_id = team_id, ver = self.m_model.m_version}, callfunc)
end
...
function M:requestRecall(team_id)
    ...
    self.m_model:getNetData("active_mining_recall_team", { team_id = team_id , ver = self.m_model.m_version}, callfunc)
end
...
function M:destroy()
    ...
    if self.m_timer2_id then
        self:removeTimer(self.m_timer2_id)
    end
