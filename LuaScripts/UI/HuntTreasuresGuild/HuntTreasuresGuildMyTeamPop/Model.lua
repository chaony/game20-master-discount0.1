local M = class("HuntTreasuresGuildMyTeamPop", LikeOO.OODataBase)

function M:onCreate()
	M.super.onCreate(self)
	self.m_transfer = "scale"
	self:getData("active_mining_my_mine",{ver = self.m_params.version})
end
...
-- 获得第一名信息
function M:getTopUserInfo()
	return self.m_data.user
end
...
function M:initData(data, is_replace)
	...
	self.mult_main_teams = table.copy(UserDataManager.hero_data:getMultTeamByKey("active_mining_defense"))
	for i = 1, 2 do
...
function M:getRegionCfgValueById(region_id, cfg_key)
	local cfg_value = nil
	if self.m_mining_region_config[self.m_version] and self.m_mining_region_config[self.m_version][tonumber(region_id)] then
		cfg_value = self.m_mining_region_config[self.m_version][tonumber(region_id)][cfg_key]
	end
	return cfg_value
end
...
function M:getMineName(team_id)
	...
	if oid ~= 0 then
		...
		if self.m_mining_region_config[self.m_version] and self.m_mining_region_config[self.m_version][tonumber(team_id)] then
			name = self.m_mining_region_config[self.m_version][tonumber(team_id)].name
		end
		return name
	end
	return name
