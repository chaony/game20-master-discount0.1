local M = class("HuntTreasuresGuildModel", LikeOO.OODataBase)

function M:onCreate()
	M.super.onCreate(self)
	self:getData("active_mining_index")
end
function M:onEnter()
	...
	UserDataManager.local_data:setLocalDataByKey("HuntTreasuresGuildVersion", self.m_version) -- 存贮活动版本号
	self:initData()
	self:getGuildAdd()
end
...
function M:getBuyCost(num)
	local num = num or 1
	local cost = 0;
	local buy_plunder = self.m_data.buy_plunder or 0 -- 已购买的次数
	--真正购买次数
	local end_num = buy_plunder + num;
	local index = 1;
	local start_num = buy_plunder + 1;
	local renovate_config = ConfigManager:getCfgByName("renovate");
	local indexTab = {1,2,3}
	if renovate_config[21] then
		local key = 1
		for k, v in pairs(renovate_config[21]) do
			indexTab[key] = k
		end
	else
		Logger.logError("renovate 表的 id 21 夺宝奇兵没配!!!")
	end
	
	for i = start_num, end_num, 1 do
		if i < indexTab[1] then
			index = indexTab[1]
		elseif i < indexTab[2] then
			index = indexTab[2]
		elseif i < indexTab[3] then
			index = indexTab[3]
		end
		cost = cost + renovate_config[18].cost[index][3];
	end
	local buy_data =  table.copy(renovate_config[18].cost[index])
	buy_data[3] = cost
	return buy_data
end
...
function M:getGuildAdd()
	...
	if self.m_mining_setting_config then
		...
		if  self.m_mining_setting_config.percent_reward and self.m_mining_setting_config.percent_reward[add_index] then
			add_reward_value = self.m_mining_setting_config.percent_reward[add_index]
		end
	end
	return add_value, add_reward_value
end
...
function M:getMineImg(mine_index)
	...
	local region_cfg = self.m_mining_region_config[self.m_version][self.m_region_id]
	local img_name = ""
