local M = class("HuntTreasuresGuildAreaPopView",LikeOO.OOPopBase)
M.m_uiName = "HuntTreasuresGuild/HuntTreasuresGuildAreaPop"
M.m_size_type = 2
local __flag_img = {is_guild = "a_mjxb_youfang", is_enemy = "a_mjxb_choujia"}
local __lan_atlas = ResourceUtil:getLanAtlas()
local _max_owners = 5
local __TAB_BTN_NODE = {
    {btn_key = "arena_btn1", lua_name = "", btn_text = "arena_btn_text1",  open = true}, -- 区域一
    {btn_key = "arena_btn2", lua_name = "", btn_text = "arena_btn_text2",  open = true,}, -- 区域二
}
function M:onEnter()
    ...
    self.m_toggle_btns = {}
    local region_id_tab = self.m_model.m_region_id_tab
    for k,v in pairs(__TAB_BTN_NODE) do
        local regionCfg = self.m_model:getRegionCfgByID(region_id_tab[k])
        self:setTextByLanKey(v.btn_text, Language:getTextByKey(regionCfg.name))
        local tog_btn = self:findToggle(v.btn_key)
        tog_btn.gameObject:SetActive(v.open)
        self.m_toggle_btns[k] = tog_btn
        local cur_tab_text = self:findText(v.btn_text)
        if k == self.m_model.m_sel_region_index then
            tog_btn.isOn = true
        end
        cur_tab_text.color = self.m_model.m_sel_region_index == k and GlobalConfig.COMMON_COLLOR.COMMON_TOGGLE_FOCUS or GlobalConfig.COMMON_COLLOR.COMMON_TOGGLE
...
function M:switchTabNode(index, keep_offset)
    for k,v in pairs(__TAB_BTN_NODE) do
        local tog_btn = self:findToggle(v.btn_key)
        tog_btn.isOn = index == k and true or false
        local cur_tab_text = self:findText(v.btn_text)
