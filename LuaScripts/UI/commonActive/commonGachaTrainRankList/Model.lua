local M = class("commonGachaTrainRankListModel", LikeOO.OODataBase)
...
function M:onCreate()
	...
	self:getData("gacha_active_rank_info", {version =self.m_params.version,start = 1,stop = 50})
end
...
function M:getRankRewards()
	local gacha_rank = ConfigManager:getCfgByName("gacha_rank")
	local rewards = gacha_rank[self.m_params.version]
	local new_tab = {}
...
function M:getRankScope(index)
	if index > 1 then
		...
		if last_rank then
			return last_rank.rank[1],last_rank.rank[2]
		end
	end
	return 0
end
