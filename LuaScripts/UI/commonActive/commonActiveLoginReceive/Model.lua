local M = class("commonActiveLoginReceiveModel", LikeOO.OODataBase)

function M:onCreate()
	M.super.onCreate(self)
	self.open_id = self.m_params.open_id or 409
...
function M:getFreeReceiveStaue(id)
	if self.m_data.war_order and self.m_data.war_order.free_received then
...
function M:getPayReceiveStaue(id)
	if self.m_data.war_order and self.m_data.war_order.pay_received  then
...
--获取显示的英雄spine
function M:getHeroSpine()
	local tongyong_warrior_reward = ConfigManager:getCfgByName("tongyong_warrior_reward")
	local hero_spine_id =  tongyong_warrior_reward[self.open_id][self.version][1].hero_spine
	local hero_skin = ConfigManager:getCfgByName("hero_skin")
	return hero_skin[hero_spine_id] or {}
end
