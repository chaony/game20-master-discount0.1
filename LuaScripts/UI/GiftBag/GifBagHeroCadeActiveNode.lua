local M = class("GifBagHeroCadeActiveNode", LikeOO.OOUIbase)
--绿林集结
M.m_uiName = "GiftBag/GifBagHeroCadeActiveNode"

function M:onCreate()
    M.super.onCreate(self)
    local function callback(response)
        if response then
            self:updateHeroNumber(response.hero_gather_active or {})
            self:refreshUI()
        end
    end
    self.m_model:getNetData("active_hero_gather_active_index", nil, callback)
end

function M:onEnter()
    self.m_active_data = self.m_params.active_data or {}
    self.m_active_ID = self.m_active_data.id or 0
    self.m_cur_version = 0
    self:updateCurVersion()
    self.m_hero_num = -1
...
function M:updateCurVersion()
    local active_tab = ConfigManager:getCfgByName("active")
    local active_cfg = active_tab[self.m_active_ID] or {}
    self.m_cur_version = active_cfg.version or 0
end

function M:updateHeroNumber(active_data)
    local active_data_version = active_data[tostring(self.m_cur_version)] or {}
    local num = active_data_version.num or 0
    local init_num = active_data_version.init_num or 0
    local num_left = num - init_num
    self.m_hero_num = math.max(num_left, 0)
end
...
function M:refreshUI()
    if self.m_hero_num == -1 then
        return
    end
    if self.m_model.hero_gather_active_received == nil then
        return
    end
    self:showUI(true)
    ...
    local hero_gather_active_tab = ConfigManager:getCfgByName("hero_gather_active")
    local cfg_data_version = hero_gather_active_tab[self.m_cur_version] or {}
    for i, v in ipairs(cfg_data_version) do
        if self.m_hero_num < v.num then
    ...
    self.end_ts = self.m_model:getActiveEndTimeByOpenId(242)
    self:updateTime()
end
...
function M:createLoopScroll()
    self.m_gift_tab = {}
    local data = self.m_model:get_hero_gather_active(self.m_hero_num, self.m_cur_version)
    if self.m_scroll_view == nil then
        ...
        local params = {
            ...
            click_func = function(index, cell_object, cell_data, click_object, click_name)
                self:updateMsg("hero_active_receive", {version = self.m_cur_version, id = cell_data.id})
            end
        }
...
function M:updateHeroItem(index, obj, data)
    ...
    if LuaBehaviour then
        ...
        local type = self.m_model:getHeroActiveCadeStatus(self.m_hero_num, self.m_cur_version, data.id)
        ...
        local cur_num = self.m_hero_num > data.num and data.num or self.m_hero_num
...
function M:onButtonClick(obj, name)
    ...
    elseif name == "help_btn" then
        ...
        params.content = cfg_data_version[1].des
