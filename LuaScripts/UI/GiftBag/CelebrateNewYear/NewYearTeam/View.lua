local M = class("NewYearTeamView", LikeOO.OOPopBase)

M.m_uiName = "GiftBag/CelebrateNewYear/NewYearTeam"
...
function M:onEnter()
    ...
    local open_data = self.m_model:getActiveCfgByOpenId(264)  
    ...
    self.team_score_text = self:findText("team_score_text")
    self.hero_obj = {}
    for i = 1, 5 do
...
function M:refreshUI( data )
    if data == nil then
        self.team_score_text.text = Language:getTextByKey("new_str_1083",0);
        --说明没有工会
        self:hideHeroObjs(-1, false)
    else
        self.team_score_text.text = Language:getTextByKey("new_str_1083", self.m_model.m_guild_score);
        self:hideHeroObjs(-1, false)
