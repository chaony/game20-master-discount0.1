local M = class("GifLimitExchangeTaskPopModel", LikeOO.OODataBase)
...
function M:onEnter()
    ...
    self.m_exchange = self.m_params.exchange
    self.m_quests = self.m_exchange.quests or {}
end
...
function M:getTaskTab()
    local new_cfg = {}
    local scroll_tab = ConfigManager:getCfgByName("exchange_quest")
...
function M:questSort(sort_tab)
    local function sortFunc(id_one, id_two)
        ...
        local get_type1 = can_get_1.status == 2 and 1 or 0
        local get_type2 = can_get_2.status == 2 and 1 or 0
