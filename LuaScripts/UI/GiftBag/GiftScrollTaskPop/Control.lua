local M = class("GiftScrollTaskPopControl", LikeOO.OOControlBase)
...
function M:getTaskReward(data)
    local function callback(response)
        if response then
            ...
            self:updateMsg("updateScrollData", response, "GiftBag.GiftScrollPanel")
    ...
    self.m_model:getNetData("scroll_receive_quest", params, callback, nil, true)
end

function M:getScrollBuy()
    local function callback(response)
        if response then
            RewardUtil:rewardTipsByData(response.reward)
            self.m_model.m_buy_times = response.buy_times
            self:updateMsg("updateScrollBuyData", response.buy_times, "GiftBag.GiftScrollPanel")
            self.m_view:refreshUI()
        end
    end
    self.m_model:getNetData("buy_scroll_goods", {vsn = self.m_model.m_version}, callback, nil, true)
end
...
function M:getAllScrollTask()
    ...
    self.m_model:getNetData("scroll_receive_all", {vsn = self.m_model.m_version}, callback)
end
