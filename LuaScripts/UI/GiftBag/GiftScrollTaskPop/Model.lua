local M = class("GiftScrollTaskPopModel", LikeOO.OODataBase)
...
function M:getTaskTab()
    local new_cfg = {}
    local draw_shop_tab = ConfigManager:getCfgByName("scroll_shop")

    local shop_cfg = draw_shop_tab[self.m_version]
    local scroll_tab = ConfigManager:getCfgByName("scroll_quest")
...
function M:get_recruit_tab()
    local new_tab = {}
    local scroll_tab = ConfigManager:getCfgByName("scroll_quest")
    local shop_tab = ConfigManager:getCfgByName("scroll_shop")
    local shop_cfg = shop_tab[self.m_version]
    table.insert(new_tab, {id = 0, cfg = shop_cfg, shop_type = true})
