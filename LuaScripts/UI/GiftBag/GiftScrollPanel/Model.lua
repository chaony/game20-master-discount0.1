local M = class("GiftScrollPanelModel", LikeOO.OODataBase)

M.point = {}

function M:onCreate()
    self:getData("scroll_index")
end
...
function M:getPanelName()
    ...
    local cur_data = open_condition_tab[118]
...
function M:getActiveCfg()
    ...
    for k,v in pairs(active_tab) do
        if v.open_id == 118 and v.version == self.m_data.version then
            return v
        end
    end
end
