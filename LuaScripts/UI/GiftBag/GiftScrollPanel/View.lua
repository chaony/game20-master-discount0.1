local M = class("GiftScrollPanelView", LikeOO.OOPopBase)

M.m_uiName = "GiftBag/GiftScrollPanel"
M.m_size_type = 2
...
function M:onEnter()
	self.m_attr_node = GameUtil:commonAttrNode(self.m_control, {mode = 9})
	...
	self:setTextByLanKey("big_title_text", "scroll_big_title")
...
function M:updateScrollItems()
	...
	if self.m_model.m_data.layer >= 20 and get_num == 25 then
...
function M:updateItemNode(index, obj, data)
	...
	if luaBehaviour then
		...
		if next(data) == nil then -- 未打开
			...
			local piece_icon = luaBehaviour:FindGameObject("piece_icon")
			if is_big == true then
				UIUtil.setScale(piece_icon.transform, 1)
			else
				UIUtil.setScale(piece_icon.transform, 0.84)	
			end
...
function M:creatItemObj()
	local item = ResourceUtil:LoadUIGameObject("GiftBag/giftscroll_item", Vector3.zero, nil)
