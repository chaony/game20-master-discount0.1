local M = class("DaySevenSPCardPopView", LikeOO.OOPopBase)

M.m_uiName = "GiftBag/DaySevenSPCardPop"
M.m_size_type = 2
...
function M:refreshUI()
    ...
    if type == 1 or type == 3 then
        ...
        self:setObjectVisible("get_day_btn", true)    --配合底图按钮倒影，只能一直显示
    end
...
function M:setSpine()
    ...
    if reward_data.data_type == RewardUtil.REWARD_TYPE_KEYS.HEROS or reward_data.data_type == RewardUtil.REWARD_TYPE_KEYS.HEROSEXT then
        ...
        if cfg then
            ...
            self:setTextByLanKey("hero_name_text",cfg.name)
            self:setImg(self:getSprite(cfg.race), ResourceUtil:getLanAtlas(), "hero_img_race")
        end
    end
end
...
function M:getSprite( type )
    ...
    elseif type == 2 then --木
        return "a_ui_zhuque"
    elseif type == 3 then --水
        return "a_ui_xuanwu"
    elseif type == 4 then --火
        return "a_ui_baihu"
    elseif type == 5 then --阳
        return "a_ui_guangming"
    elseif type == 6 then --阴
        return "a_ui_heian"
    elseif type == 7 then --元
        return "a_ui_yuan"
    elseif type == 8 then --sp
        return "a_ui_sp"
    end
end
