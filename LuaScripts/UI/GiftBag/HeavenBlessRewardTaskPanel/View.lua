local M = class("HeavenBlessRewardTaskPanelView", LikeOO.OOPopBase)

M.m_uiName = "GiftBag/HeavenBlessRewardTaskPanel"
M.m_size_type = 2
...
function M:onEnter()
    ...
    self:refreshUI()
    self:setObjectVisible("all_get_btn", false)
    ...
    RedPointUtil:saveLocalRedPointFreshTime("heaven_bless_reward_once")
end

function M:refreshUI()
    self:setObjectVisible("tab_tog",false)
...
function M:createTagLoopScroll()
    local data = self.m_model.m_quest
    --for i = 1, self.m_model.m_days do
    --    table.insert(data, {})
    --end
    self.m_tag_tab = {}
...
function M:update_tag(index, obj, data)
    ...
    if luaBehaviour then
        ...
        if index == self.m_model.m_select_index then
            ...
            day_text.color = GlobalConfig.COMMON_COLLOR.COMMON_2
        end
    end
end
...
function M:SortData(data)
    ...
    for k,v in ipairs(data) do
        if (v.shop_type == true and v.log_type and v.log_type == true and (self.m_model:checkLogin() == 1 or self.m_model:checkLogin() == -1 )) or 
                (v.shop_type == true and not v.log_type and (v.cfg.time - self.m_model:checkBuy()) <= 0)then
...
function M:createLoopScroll()
    self.m_gift_tab = {}
    --local data = self.m_model.m_quest[self.m_model.m_select_index]
    local data_ = self.m_model.m_quest[self.m_model.m_day]
    local data = self:SortData(data_)
                ...
                if click_name == "ok_btn" then
					if cell_data.shop_type == true then
                        if cell_data.log_type == true then
                            if self.m_model:checkLogin() == 0 then
						...
						local data = self.m_model:getQuestStuage(cell_data.id)
    ...
    if luaBehaviour then
        ...
        if cell_data.shop_type == true then
            ...
            if cell_data.log_type and cell_data.log_type == true then
                if self.m_model:checkLogin() == 1 then
                    LuaBehaviourUtil.setTextByLanKey(luaBehaviour, "btn_text", "gf_str_0040") --已签到
                    LuaBehaviourUtil.setImg(luaBehaviour, "ok_btn", "a_ui_currency_btn_small_2", "common_ui")
                    btn_img.material = self.m_gray_img.material
                elseif self.m_model:checkLogin() == -1 then
                    LuaBehaviourUtil.setTextByLanKey(luaBehaviour, "btn_text", "gf_str_0150") --已结束
                    LuaBehaviourUtil.setImg(luaBehaviour, "ok_btn", "a_ui_currency_btn_small_2", "common_ui")
                    btn_img.material = self.m_gray_img.material
                else
                    LuaBehaviourUtil.setTextByLanKey(luaBehaviour, "btn_text", "gf_str_0039") --签到
                    LuaBehaviourUtil.setImg(luaBehaviour, "ok_btn", "a_ui_currency_btn_small_2", "common_ui")
