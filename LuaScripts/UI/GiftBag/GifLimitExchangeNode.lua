local M = class("GifLimitExchangeNode", LikeOO.OOUIbase)
--限时兑换
M.m_uiName = "GiftBag/GifLimitExchangeNode"
...
function M:onEnter()
    ...
    self.id = 0
    self.m_version = 1
    self:setTextByLanKey("jump_text","buy_growup_get")
    UserDataManager:removeRedDotByKey("exchange")
    self:showUI(false)
end
...
function M:refreshUI()
    if self.m_model.m_exchange_data == nil or next(self.m_model.m_exchange_data) == nil then
        return
    end
    self:showUI(true)
    local active_tab = ConfigManager:getCfgByName("active")
    for k,v in pairs(self.m_model.m_exchange_data.actives) do
    ...
    if self.m_model.m_exchange_data.exchange and self.m_model.m_exchange_data.exchange.version then
        self.m_version = self.m_model.m_exchange_data.exchange.version
    else
        self.m_version = active_cfg.version
    end
    self.pop_name = Language:getTextByKey(active_cfg.name)
    self.exchange_cfg = self.m_model:get_exchange_cfg(self.m_version)
    self:updateTime()
    self:createLoopScroll()
    self:setSpine()
    local data = self.m_model:get_exchange_limit_cfg(self.m_version)
    if #data > 0 then
        local need_data = data[1].need_reward[1]
        local consume_data = RewardUtil:getProcessRewardData(need_data)
        self:setTextByLanKey("cons_text", "gf_str_0052", consume_data.name)
        self:setImg(consume_data.icon_name, consume_data.atlas_name, "cons_img")
        self:setTextByLanKey("cons_num", consume_data.user_num)
    end
    local red_bl = RedPointUtil:isFuncRedPointById(154)   
    --self:setObjectVisible("jump_red_point", red_bl == true)
end
    ...
    if self.m_scroll_view == nil then
        ...
        local params = {
            ...
            click_func = function(index, cell_object, cell_data, click_object, click_name) -- 点击回调
                ...
                local data = self.m_model:getExchangeData(self.m_version, cell_data.id)
                ...
                local params = {
                    text = Language:getTextByKey("predestined_str_011",need_data.data_num,need_data.name, out_data.data_num,out_data.name).."?",
                    on_ok_call = function ()
                        self:updateMsg("exchange",{id = cell_data.id, version = self.m_version})
                    end
                }
...
function M:updateRewardItem(obj, cfg)
    ...
    if LuaBehaviour then
        local need_item = LuaBehaviour:FindGameObject("need_item")
        local reward_parent = LuaBehaviour:FindGameObject("reward_node") 
        local reward_btn = LuaBehaviour:FindImage("reward_btn")
        local data = self.m_model:getExchangeData(self.m_version, cfg.id)
        local can_change = true
        local num = cfg.times - data
        GameUtil:updateItemElement(need_item, cfg.need_reward[1], true, true)
        GameUtil:createRewards(reward_parent.transform, cfg.out_reward, true, true)
...
function M:showUI(bl)
	...
	self:setObjectVisible("cons_text", bl)
	self:setObjectVisible("cons_img", bl)
    self:setObjectVisible("cons_num", bl)
end
