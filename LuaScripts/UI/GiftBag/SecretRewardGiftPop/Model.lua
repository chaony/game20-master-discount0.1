local M = class("SecretRewardGiftPopModel", LikeOO.OODataBase)
...
function M:onCreate()
    ...
    self:getData("secret_gift_index")
end

function M:onEnter()
    self.m_version = self.m_data.version
    StatisticsUtil:doPointActive(288,self.m_version)
...
function M:get_scroll_shop_cfg()
    ...
    local new_tab = table.copy(scroll_shop_tab[self.m_version or 1])
...
function M:getActivityGiftData(id)
    if self.m_data and self.m_data.activity_log then
