local M = class("MoonShadowDateView",LikeOO.OOPopBase)

M.m_uiName = "MoonShadow/MoonShadowDate"
M.m_size_type = 1
M.m_iphoneXAdapter = true

function M:onEnter()
	self:setTextByLanKey("close_title_text", "moon_shadow_str_002")
	...
	self:refreshUI()
	RedPointUtil:saveLocalRedPointFreshTime("MoonShadowDate")
end
		...
		local params = {
			...
			click_func = function(index, cell_object, cell_data, click_object, click_name)
				...
				elseif click_name == "jifen_obj" then
					GameUtil:lookInfoTips(self.m_control, {click_transform = click_object.transform, msg = Language:getTextByKey("xiaying_text_0001")})
				end
			end
		}
...
function M:updateZhuanjiDetailLoopScrollCell(index, cell_object, cell_data)
	...
	GameUtil:createRewards(reward_node, cell_data.reward, true, true, nil, 0.85)
...
function M:updateZhuanjiLoopScrollCell(index, cell_object, cell_data)
	...
	GameUtil:updateResourcesImg(bg_img, "Texture/moon_shadow/a_yycs_zhuanji")
	...
	elseif cell_data.status == 2 then	--当前关
		GameUtil:updateResourcesImg(bg_img, "Texture/moon_shadow/a_yycs_xuanzhongzhuanji")
	...
	elseif cell_data.status == 0 then	--未开启
		GameUtil:updateResourcesImg(bg_img, "Texture/moon_shadow/a_yycs_weijiesuozhuanji")
...
function M:updateRewardBox()
	...
	for i=1,box_num do
		...
		local cfg = data.cfg
		local task_box = GameUtil:createPrefab("MoonShadow/MoonShadowRewardBox", box_trans)
