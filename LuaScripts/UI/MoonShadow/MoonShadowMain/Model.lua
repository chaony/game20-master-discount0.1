local M = class("MoonShadowMainModel", LikeOO.OODataBase)

function M:onCreate()
	M.super.onCreate(self)
	self:getData("mood_shadow_index")
end
...
function M:onEnter()
	...
	self.m_activity_openID_MoonShadow = 235
	self.m_activity_openID_Date = 237
	self.m_activity_openID_Gift = 236
	self.m_activity_openID_Battle = 238
	self.m_activity_openID_name_cache = {
		[self.m_activity_openID_MoonShadow] = Language:getTextByKey("moon_shadow_str_001"),
		[self.m_activity_openID_Date] = Language:getTextByKey("moon_shadow_str_002"),
		[self.m_activity_openID_Gift] = Language:getTextByKey("moon_shadow_str_003"),
		[self.m_activity_openID_Battle] = Language:getTextByKey("moon_shadow_str_004"),
	}
	self:setCurrentDay()
end
...
function M:setCurrentDay()
	...
	local activity_moon_shadow_data = self:getActivityData(self.m_activity_openID_MoonShadow)
	if activity_moon_shadow_data.start_ts then
		start_time = activity_moon_shadow_data.start_ts
	end
...
function M:getVersion(open_id)
	open_id = open_id or self.m_activity_openID_MoonShadow
...
function M:getLockTip(activity_data)
	...
	return Language:getTextByKey("moon_shadow_str_005", star_date_str, end_date_str)
end
...
function M:hasMoonShadowDateRedPoint()
	--是否有没领取的任务奖励
	...
	--是否有没领取的宝箱
	local moon_shadow_reward = ConfigManager:getCfgByName("mood_shadow_reward")[self:getVersion(self.m_activity_openID_Date)] or {}
...
function M:hasMoonShadowGiftRedPoint()
	local moon_shadow_gift_tab = ConfigManager:getCfgByName("mood_shadow_gift")[self:getVersion(self.m_activity_openID_Gift)] or {}
	for _, index_item in pairs(self.m_data.gift_data or {}) do
		for data_index, data_item in pairs(moon_shadow_gift_tab) do
