...
M.m_uiName = "Item/ItemTalinsDetail"
M.m_size_type = 2
...
--刷新符篆
function M:updateTalinsView()
	local data = self.m_model.m_show_data
	local cfgdata = UserDataManager.talis_data:getTalisSuitConfigByCid(data.data_id)
	...
	for i = 1,3 do
		self:setObjectVisible("left_property_slider_"..i,false)
			--设置属性			
		self:setTextByLanKey("attr_count_text"..i, "talisman_text_0039")
	end
    --特殊处理
	if cfgdata.quality < 14 then 
		self:setObjectVisible("attr_count_text3",false)
	end
	--设置战力
	self:setTextByLanKey("zhanli_text",  Language:getTextByKey("talisman_text_0035"))
	self:setTextByLanKey("combat_text", "talisman_text_0040")

	--共鸣效果
	self:setTextByLanKey("eqp_des_title_text1","talisman_text_0018")
