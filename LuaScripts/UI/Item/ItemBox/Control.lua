local M = class("ItemBoxControl",LikeOO.OOControlBase)
...
function M:onHandle(msg , data)
    if msg == 99999 then    -- 返回
        self:closeView()
    elseif msg == "choice_item" then
        self:useItem(data.index)
    elseif msg == "choice_btn" then
        if self.m_model.m_cur_select_index ~= 0 then
            self:useItem(self.m_model.m_cur_select_index)
        else
            GameUtil:lookInfoTips(self, {msg = Language:getTextByKey("new_str_0810"), delay_close = 2})
        end    
    end
end
...
function M:useItem(index)
    ...
    local params = {item_id = item_id, item_num = item_num, item_index = index - 1}
