local M = class("YinTowerDeffDetailView",LikeOO.OOPopBase)

M.m_uiName = "YinTower/YinTowerDeffDetail"
M.m_size_type = 2
...
function M:onEnter()
	...
	self:setTextByLanKey("qw_title_text","four_tower_str_0017")
	self:setTextByLanKey("common_title_text",title_str)
end
...
function M:refreshUI()
	...
	if self.m_model:getDesByKey("des") then
		tip_lan = self.m_model:getDesByKey("des")
	end
	...
	self:setObjectVisible("ok_btn", not(self.m_model:isMaxFloor()))
...
function M:updateLegacyInfo()
	...
	self:setObjectVisible("legacy_node",heirloom and true or false)
	if heirloom == nil then
		return
	end
