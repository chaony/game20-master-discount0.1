local M = class("GuJianQiTanMazeHeroSelectSubView",LikeOO.OOPopBase)

M.m_uiName = "GuJianQiTan/GuJianQiTanMazeHeroSelectSub"
M.m_size_type = 2
...
function M:onEnter()
	...
	self:setTextByLanKey("tips_text", "xue_gu_jian_qi_tan_str_001")
	self:setTextByLanKey("common_title_text", maze_cell_type_item.name or "???")
	self:setTextByLanKey("ok_btn_text", "xue_gu_jian_qi_tan_str_002")
		...
		local params = {
			...
			update_cell = function(index, cell_object, cell_data)
				...
				if luaBehaviour then
					...
					LuaBehaviourUtil.setTextByLanKey(luaBehaviour, "combat_num_text", tostring(data.hero_data.combat))
