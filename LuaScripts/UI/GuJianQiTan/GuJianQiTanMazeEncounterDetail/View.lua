local M = class("GuJianQiTanMazeEncounterDetailView",LikeOO.OOPopBase)

M.m_uiName = "GuJianQiTan/GuJianQiTanMazeEncounterDetail"
M.m_size_type = 2
...
function M:refreshUI()
	...
	local team_cfg = self.m_model:getTeamData();
	...
	self:setTextByLanKey("common_title_text", team_cfg.name)
	...
	GameUtil:updateResourcesImg(self.m_encounter_img, "Texture/biography/" .. tostring(event_data.pic))
	self:updateSelectDramaLoopScroll()
end

function M:refreshSelectInfo(id)
	self.m_model.m_id = id;
	...
	--local show_btn_flag = encounter_cfg_item.event_battle > 0
	local show_btn_flag = false
	self:setObjectVisible("battle_btn",show_btn_flag)
	self:setObjectVisible("event_loopscroll", not show_btn_flag)
	self:updateSelectDramaLoopScroll()
end
