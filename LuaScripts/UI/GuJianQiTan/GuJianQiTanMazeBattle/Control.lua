local M = class("GuJianQiTanMazeBattleControl",LikeOO.OOControlBase)

function M:onEnter()
    self.m_guide_file_name = "UI.GuJianQiTan.GuJianQiTanMazeBattle.Guide"
end
...
function M:onHandle(msg , data)
    ...
    elseif msg == "speed_btttle" then
        ...
        local moveFinish = {
            callback = function()
                static_rootControl:updateMsg("speed_btttle",cell_data,"GuJianQiTan.GuJianQiTanMaze");
            end
        }
        if self.m_model.m_callBack ~= nil then
            self.m_model.m_callBack( moveFinish )
