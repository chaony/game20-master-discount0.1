...
local M = class("PetardGiftBagNewControl",LikeOO.OOControlBase)
...
function M:onHandle(msg , data)
    if msg == 99999 then
        self:updateMsg("refreshAllRedDot" ,nil ,"PetardActivity.PetardMain")
    ...
    elseif msg == "explain_btn" then
        ...
        self:openView("Pops.CommonHelpPop", { title = Language:getTextByKey(mainActiveData.name), content = Language:getTextByKey("tid#ChargeturnDes_1") })
    ...
    elseif msg == "buy" then
        self:buyForSDK(data)
    elseif msg == "btn_buyBoxBtn" or msg == "btn_buySkinBtn" then
        if self.m_model:checkClothGift() == 0 then 
            self:buyForSDK(self.m_model.m_hero_gift_cfg.charge_id)
        end
    end
end
