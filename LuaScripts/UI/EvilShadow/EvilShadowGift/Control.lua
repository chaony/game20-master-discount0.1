local M = class("EvilShadowGiftControl",LikeOO.OOControlBase)
...
function M:onHandle(msg , data)
    if msg == 99999 then    -- 返回
        self:updateMsg("refreshRedPointGift", nil, "EvilShadow.EvilShadowMain")
...
function M:buyForSDK(data)
    if data then
        ...
        if data.price == 0 then
            ...
            self.m_model:getNetData("evil_shadow_receive_free_gift", params, netCallback)
        --付费    
