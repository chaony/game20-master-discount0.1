local M = class("EvilShadowDateView",LikeOO.OOPopBase)

M.m_uiName = "EvilShadow/EvilShadowDate"
M.m_size_type = 1
M.m_iphoneXAdapter = true

function M:onEnter()
	self:setTextByLanKey("close_title_text", "evil_shadow_str_002")
	...
	self:refreshUI()
	RedPointUtil:saveLocalRedPointFreshTime("EvilShadowDate")
end
		...
		local params = {
			...
			click_func = function(index, cell_object, cell_data, click_object, click_name)
				...
				elseif click_name == "jifen_obj" then
					GameUtil:lookInfoTips(self.m_control, {click_transform = click_object.transform, msg = Language:getTextByKey("meiying_text_0001")})
				end
			end
		}
...
function M:updateZhuanjiLoopScrollCell(index, cell_object, cell_data)
	...
	local bg_img_name = "a_mycs_zhuanji"
	...
	elseif cell_data.status == 2 then	--当前关
		bg_img_name = "a_mycs_xuanzhongzhuanji"
	...
	elseif cell_data.status == 0 then	--未开启
		bg_img_name = "a_mycs_weijiesuozhuanji"
	...
	LuaBehaviourUtil.setImg(luaBehaviour, "bg_img", bg_img_name, "coach_ui")
