local M = class("EvilShadowMainControl",LikeOO.OOControlBase)
...
function M:onHandle(msg , data)
    ...
    elseif msg == "date_btn" then
        if self:checkOpenStatus(self.m_model.m_activity_openID_Date) then
            self:openView("EvilShadow.EvilShadowDate", {version = self.m_model:getVersion(self.m_model.m_activity_openID_Date), current_day = self.m_model:getCurrentDay(), score = self.m_model:getScore(), score_done = self.m_model:getScoreDone(), task_detail_data = self.m_model:getTaskDetailData()})
        end
    elseif msg == "gift_btn" then
        ...
        if self:checkOpenStatus(self.m_model.m_activity_openID_Gift) then
            self:openView("EvilShadow.EvilShadowGift", {version = self.m_model:getVersion(self.m_model.m_activity_openID_Gif), 
    ...
    elseif msg == "battle_btn" then
        ...
        if self:checkOpenStatus(self.m_model.m_activity_openID_Battle) then
            self:openView("EvilShadow.EvilShadowBattle", {version = self.m_model:getVersion(self.m_model.m_activity_openID_Battle), 
                                                          current_day = self.m_model:getCurrentDay(),
    ...
    elseif msg == "share_btn" then
        --self:openView("SharePoster", {version = 1, sort = 2, picture_id = 4, evil_shadow_flag = true, callback = function() self:updateMsg("refreshRedPointDate") end})--分享回来，刷新数据，确保月影之约中任务数据是最新的
        self:openView("EvilShadow.EvilShadowExchangeShop", {version = self.m_model:getVersion(), exchange_done_data = self.m_model:getExchangeDoneData()})
    elseif msg == "refreshRedPoint"  or msg == "refreshRedPointExchange" then
    ...
    elseif msg == "refreshRedPointDate" or msg == "refreshRedPointGift" or msg == "refreshRedPointBattle" then
        ...
        self.m_model:getNetData("evil_shadow_index", nil, netCallback)
    end
end
