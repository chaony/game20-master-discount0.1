local M = class("MazeStageChoiceModel", LikeOO.OODataBase)
...
function M:getUnlockMapGroupId(cur_group_cfg)
	local cur_stage = UserDataManager:getCurStage();
	--最大的组是否解锁
	if cur_stage < cur_group_cfg.stage then
		return false
	end
	local cur_season = UserDataManager:getCurSeason()
	if cur_season < cur_group_cfg.season then
		return false
	end
	if self.max_floor < cur_group_cfg.floor_id[1] then
		return false;
	end
	return true
end

function M:getShowData2()
	--本地数据
	self.maze_map_group = ConfigManager:getCfgByName("maze_map_group");
	local result = {}
	local unlock_name = ""
	for k,v in ipairs(self.maze_map_group) do
		...
		local is_unlock = self:getUnlockMapGroupId(v)
		if k > 0  then
			...
			if is_unlock then
				...
				v.is_unlock_level_name = unlock_name;
				v.is_complete = false
				break
			end
		end
