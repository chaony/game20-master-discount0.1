local M = class("MazeStageChoiceControl",LikeOO.OOControlBase)

function M:onEnter()
    self.m_guide_file_name = "UI.MazeStage.MazeStageChoice.Guide"
end
...
function M:requestChooseFloor( data )
    local function netCallback(response)
        ...
        self:openView("MazeStage",{data = response});
        self:closeView();
    end
