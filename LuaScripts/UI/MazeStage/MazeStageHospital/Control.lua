local M = class("MazeStageHospitalControl",LikeOO.OOControlBase)
            ...
            callback = function()
                if cell_data.status == 0 then
                    ...
                    if cell_data.type == 5 then --5.医馆
                        self:updateMsg("maze_add_blood", {data = cell_data}, "MazeStage")
                    elseif cell_data.type == 6 then -- 6.药王庙
                        self:updateMsg("maze_revive_one", {data = cell_data}, "MazeStage")
                    end
                end
            end
        }
