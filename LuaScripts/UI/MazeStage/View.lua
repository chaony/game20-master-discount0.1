local M = class("MazeStageView",LikeOO.OOPopBase)

M.m_uiName = "MazeStage/MazeStage"
...
function M:refreshUI()
	...
	local max_floor_data = maze_floor[max_floor_id];
	local finish_reward = max_floor_data.finish_reward and max_floor_data.finish_reward or {}
	if self.m_model:isFirstComplete() then
		--参数传空 ui隐藏
		self:refreshFinishRewardNode()
	else
		self:refreshFinishRewardNode(finish_reward, max_floor_id)
	end
	...
	self:timeUpdate()
	local box_num = self.m_model:getNotExploredCellNum(14)
	self:setTextByLanKey("box_num_text", "new_str_0693", box_num)
	local businessman_num = self.m_model:getNotExploredCellNum(7)
...
function M:refreshFinishRewardNode(data, floor_id)
	...
	if data and next(data) then
		...
		self:setTextByLanKey("reward_des_text", "new_str_0955")
	else
