---@class MultiFormationNode:OOUIbase
--- 多阵容
local M = class("MultiFormationNode",LikeOO.OOUIbase)

M.m_uiName = "Formation/MultiFormationNode"
...
function M:refreshUI()
    if self.m_show_status == 1 then
        ...
        local formation_data_item = formation_data[tostring(self.m_sel_formation_index)] or {}
        local team = self.m_main_team or formation_data_item.team
        team = team or {}
        local no_hero = GameUtil:teamNoHero(team)
        local name_str = nil
        if no_hero then
            name_str = Language:getTextByKey("new_str_0551") .. tostring(self.m_sel_formation_index)
        else
        ...
        local team_heros_data, combat = self:getHerosDataByTeam(team)
    ...
    if self.m_formation_loop_scroll_view == nil then
        ...
        local params = {
            ...
            update_cell = function(index, cell_object, cell_data)
                ...
                local team_heros_data, combat = self:getHerosDataByTeam(cell_data.team)
...
function M:updateHeroLoopScroll()
    ...
    local sel_screen = self.m_sel_screen or 1
    local data = nil
...
function M:showTeamEdit(index, main_team)
    self.m_show_status = 2
    self.m_sel_formation_index = index or self.m_sel_formation_index
    self.m_main_team = main_team or self.m_main_team
