local M = class("CommonTimesPopControl", LikeOO.OOControlBase)
...
function M:onHandle(msg , data)
	...
	elseif msg == "add_btn" then
		if self.m_model.m_cost * (self.m_model.m_buyNum + 1) > self.m_model.m_yuanbao_count  then
			GameUtil:lookInfoTips(self, {msg = Language:getTextByKey("qi_xi_038"), delay_close = 2})
			return
		elseif self.m_model.m_buyNum + 1 > self.m_model.m_limit_num then
			GameUtil:lookInfoTips(self, {msg = Language:getTextByKey("wind_clouds_red_packet_text_00018"), delay_close = 2})
			return
		end
	...
	elseif msg == "max_btn" then
		if self.m_model.m_cost * (self.m_model.m_buyNum + 10) > self.m_model.m_yuanbao_count then
			GameUtil:lookInfoTips(self, {msg = Language:getTextByKey("qi_xi_038"), delay_close = 2})
			return
		elseif self.m_model.m_buyNum + 10 > self.m_model.m_limit_num then
			GameUtil:lookInfoTips(self, {msg = Language:getTextByKey("wind_clouds_red_packet_text_00018"), delay_close = 2})
			return
		end
