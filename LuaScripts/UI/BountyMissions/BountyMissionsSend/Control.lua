--悬赏列表
local M = class("BountyMissionsSendControl",LikeOO.OOControlBase)

function M:onEnter()
    self.m_guide_file_name = "UI.BountyMissions.BountyMissionsSend.Guide"
end
...
function M:onHandle(msg, data)
    ...
    elseif msg == "send_btn" then    
        self:sendHero()
    elseif msg == "quck_send_btn" then --一键上阵
...
function M:sendHero()
    if self.m_model:checkNumCondition() and self.m_view:checkRaceCondition() then
        ...
        local function callfunc(data)
            self:updateMsg("resresh", {quest_id =  self.m_model.m_task_id },"BountyMissions")
