local M = class("YinTowerRewardPopControl",LikeOO.OOControlBase)

function M:onEnter()
    self.m_guide_file_name = "UI.YinTower.YinTowerRewardPop.Guide"
    ...
    elseif msg == "replay_btn" then
        ...
        if self.m_model.finish == true then
            ...
            self.m_model:getNetData("five_element_over", nil, function( data )
                ...
                self:updateMsg("five_over", {floor = data.floor, finish = data.finish}, "YinTower")
                self:closeView()
            end)
        else
            self:closeView()
    ...
    elseif msg == "mopping_btn" then
        ...
        self.m_model:getNetData("five_element_sweep", nil, function( data )
            ...
            TimeTools:delayTimeUnity(0.5, function()
                --发送到五行阵主界面更新奖励
                static_rootControl:updateMsg("five_mopping", params , "YinTower")
            end)
        end)
    end
end
