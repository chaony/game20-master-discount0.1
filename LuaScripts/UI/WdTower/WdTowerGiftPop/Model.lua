local M = class("WdTowerGiftPopModel", LikeOO.OODataBase)
...
function M:onEnter()
	...
	self.m_max_score = self.m_data.max_score or 0 -- 当前打到的最高分
...
function M:getGiftStatus()
	local cur_opencontidion_id = 294
	local temp_push_data = {}
	local choice_gifts = UserDataManager.m_choice_gifts or {}
	local status = false
	for i, v in pairs(choice_gifts) do
...
function M:getGiftsData()
	...
	local tower_reward_active_cfg = ConfigManager:getCfgByName("wdtower_gift")
...
function M:getCanGetRewardDay(reward_data)
	...
	local day = 1
	if self.m_cur_day >= reward_data.day and self.m_max_score >= reward_data.num  then
