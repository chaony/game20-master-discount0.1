local M = class("RacconGiftControl",LikeOO.OOControlBase)
...
function M:onHandle(msg , data)
    ...
    elseif msg == "box_reward" then
        audio:SendEvtUI("UI_Pay")
        local score = data.data.score or 0
        self:getHotReward(score)
...
function M:switchTabBtn(index)
    ...
    if self.m_model:getSelectedIndex() ~= index then
        self.m_model:setTabIndex(index)
        if next(self.m_model:getCurNetData()) then
            self.m_model:initGiftData()
...
function M:getHotReward(score)
    ...
    self.m_model:getNetData("active_common_gift_recv_hot", {score = score, open_id = self.m_model:getOpenID(), vsn = self.m_model:getVersion()}, netCallback)
end
