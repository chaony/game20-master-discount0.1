local M = class("RacconTinShotModel", LikeOO.OODataBase)
...
function M:onEnter()
    ...
    --活动版本号
    self.m_actives = UserDataManager:getActivesDataByOpenId(314)
    self.version = 0
    if self.m_actives then
        self.version = self.m_actives.version;
    end
    self.m_msg_data = self.m_data.big_win_msg or {}
    self.m_daily_times = self.m_data.daily_times or 0 --次数
    self.m_total_times = self.m_data.total_times or 0 --总次数
    self.m_cfg_id_tab = {}
    ...
    self.m_draw_gifts = self.m_data.draw_gifts or {}
    self.times = self.m_data.draw_times;
    self.big_reward_index = 1 --大奖轮播
    self.main_gacha_cfg = self:getMainGachaCfg()
    self.m_need_item = self.main_gacha_cfg.gacha[1]
    self.m_show_item = {}
    if self.main_gacha_cfg.gacha then
        table.insert( self.m_show_item, self.main_gacha_cfg.gacha[1][2] )
    end
    if self.main_gacha_cfg.gacha_item then 
        table.insert( self.m_show_item, self.main_gacha_cfg.gacha_item[1][2] )
    end
end
...
function M:drawFromServer( time, call_back )
    self:getNetData("gacha_active_draw",{ version = self.version, times = time }, function( data )
...
function M:getCanUseReward()
    ...
    for i, cfg_id in ipairs(self.m_cfg_id_tab) do
        ...
        if v and v.is_show and v.is_show == 1 then
            if #rewards < 3 then
...
function M:getMainGachaCfg()
    ...
    return main_gacha_tab[self.version]
end
