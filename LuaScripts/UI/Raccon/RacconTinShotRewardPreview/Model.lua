local M = class("RacconTinShotRewardPreviewModel", LikeOO.OODataBase)
...
function M:getBigReward()
    local scroll_tab = nil
    if self.m_open_type == "collect" then
        scroll_tab = ConfigManager:getCfgByName("raccon_card_gacha")
    else
        scroll_tab = ConfigManager:getCfgByName("spring_festival_gacha")
    end
