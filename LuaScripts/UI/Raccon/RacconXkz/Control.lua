local M = class("RacconXkzControl",LikeOO.OOControlBase)
...
function M:openXkzDetail(hero_index)
    ...
    if is_lock then
        ...
        self:openView("Raccon.RacconXkzDetail", {hero_index = hero_index,
...
function M:getBoxReward(box_id)
    ...
    self.m_model:getNetData("raccon_chapter_recv", {score_id = box_id, chapter_id = self.m_model.m_cur_index}, netCallback)
end
...
function M:refreshIndex()
    ...
    self.m_model:getNetData("raccon_index", nil, receivetCallback)
end
