local M = class("FivelinesNewModel", LikeOO.OODataBase)

function M:onCreate()
	M.super.onCreate(self)
	self:getData("four_tower_index")
end

function M:onEnter()
	self.current_flood = self.m_data.floor
...
function M:getTowerStageData()
	local nfour_tower_stage = ConfigManager:getCfgByName("nfour_tower_stage")
...
function M:getHeirloomCombatAddRatio()
	local team = UserDataManager.hero_data:getTeamByKey("four_tower") or {}
...
function M:IsNextFlood()
	local multiple = self:getMultiple()
	if multiple == 1 then
	...
	for i, v in pairs(events) do
		if v.type == 2 then
			for i, v in pairs(v.heros) do
				UserDataManager.local_data:setLocalDataByKey("fourDifficulty",v.lv)
				return v.lv
			end
		end
	end
	return UserDataManager.local_data:getLocalDataByKey("fourDifficulty",1)
end
