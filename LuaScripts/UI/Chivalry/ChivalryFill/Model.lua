local M = class("ChivalryFillModel", LikeOO.OODataBase)
...
function M:onEnter()
    ...
    self.open_num = self.m_data.version - 6
    local active_datas = self:getActiveCfgByOpenId(self.open_id)
    if active_datas then
        local end_time = GameUtil:stringToTimesTamp(active_datas.end_time) --活动结束时间
        local cur_tim = UserDataManager:getServerTime() --当前时间
        if cur_tim > end_time then
            self.open_num = self.m_data.version
        end
    end
end
...
--获取活动数据
function M:getActiveData(version)
    local main_gacha = ConfigManager:getCfgByName("main_gacha")
    self.active_datas = main_gacha[version]
    return main_gacha[version]
end

function M:getOneCost()
    local active_datas = self:getActiveData(self.m_data.version)
    if active_datas then
...
function M:getCanUseReward()
    local gacha_data = self.spring_festival_gacha[self.m_data.version] or {}
    local rewards = {}
...
function M:getBigRewardStr()
    ...
    for i = #self.m_data.big_win_msg,1,-1 do
        ...
        if gacha_data then
            ...
            cur_str = Language:getTextByKey("chivalry_text_0011", server_name, name, item_name)
...
function M:getShowContent()
    ...
    if self.show_stage == nil then
        self.show_stage = 1
    end
    return legend_monument[self.show_stage+6]
end
