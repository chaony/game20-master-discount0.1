local M = class("ThreeHeroesFiveGallantsStoreDramaControl", LikeOO.OOControlBase)

function M:onEnter()
	self.m_guide_file_name = "UI.ThreeHeroesFiveGallants.ThreeHeroesFiveGallantsStoreDrama.Guide"
...
function M:runStoryEvent( data )
	...
	if data.type ~= nil and data.type ~= "" then
		...
		elseif data.type == 5 then
			...
			if next(param) ~= nil then
				...
				self.m_view:setTextByLanKey("white_text", word_talk)
	...
	if data.long_cg ~= nil and data.long_cg ~= "" and next(data.long_cg) then
	...
	elseif msg == "select_drama_Item" then
		if data.cell_data.can_choise then
			...
			if data.cell_data.no_choise_tips == nil or data.cell_data.no_choise_tips == 0 then
				...
				if self.m_model.Map2DControl ~= nil and self.m_model.Map2DControl.curMap2D ~= nil then
					self.m_model.Map2DControl.curMap2D:playPlot(data.cell_data.no_choise_tips, "no_choise_plot", nil)
				end
			end
		end
