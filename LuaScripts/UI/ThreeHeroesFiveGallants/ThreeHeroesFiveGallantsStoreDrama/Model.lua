local M = class("ThreeHeroesFiveGallantsStoreDramaModel", LikeOO.OODataBase)
...
function M:onEnter()
	...
	self.dialogue_type = self.m_params.dialogue_type or 4
	self.m_bgm = ""
	if not ids then
		...
		if is_guide then
			...
			elseif self.dialogue_type == GlobalConfig.WORLD_MAP_EVENT.REGIONAL_EVENT then -- 地图普通事件对话
				...
				self.m_dialogue_team_name = "active_plot_event"
		...
		local team_custom = ConfigManager:getCfgByName("active_plot_taskteam")
...
function M:getNpcCfg()
	...
	--	end
	--end
	npc_cfg = ConfigManager:getCfgByName("active_plot_npc")
	return npc_cfg
end
