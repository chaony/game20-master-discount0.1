...
--大地图事件
---@class ThreeHeroesFiveGallantsMapEvent @
local M = class("ThreeHeroesFiveGallantsMapEvent")

--初始化事件
function M:init( data, event_type, Map2DControl)
    self.eventData = data;
    self.event_type = event_type
    self.Map2DControl = Map2DControl
end
        ...
        if event_before > 0 then
            if self.event_type == GlobalConfig.WORLD_MAP_EVENT.REGIONAL_EVENT and self.eventData.isArticle ~= true then
                if self.eventData.option_switch == 1 then
                    local article_option = ConfigManager:getCfgByName("active_plot_option")
                    local choise = {}
                    local choise_id = {}
...
function M:talk(talk_id, choise, choise_id, can_choise, no_choise_tips)
    ...
    static_rootControl:openView("ThreeHeroesFiveGallants.ThreeHeroesFiveGallantsStoreDrama", {Map2DControl = self.Map2DControl, dialog_id = data.talk_id, callback = callback, choise = data.choise, choise_id = data.choise_id, can_choise = can_choise, no_choise_tips = no_choise_tips, dialogue_type = data.dialogue_type, waitForChoise = data.waitForChoise}, nil , true)
end
