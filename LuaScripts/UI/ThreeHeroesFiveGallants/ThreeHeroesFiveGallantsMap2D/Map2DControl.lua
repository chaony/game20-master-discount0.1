...
function M:openMap2D( id , callback)
    self:changeScene(id, function()
        ...
        self.curMap2D = require("UI.ThreeHeroesFiveGallants.ThreeHeroesFiveGallantsMap2D.Map2D").new(self.m_control, {Map2DControl = self, map_id = id, is_first = isFirst, callback = callback})
...
function M:back()
    if self.stack:hasData() then
        ...
        self:changeScene(cur_map_id, function()
            ...
            self.curMap2D = require("UI.ThreeHeroesFiveGallants.ThreeHeroesFiveGallantsMap2D.Map2D").new(self.m_control, {Map2DControl = self, map_id = cur_map_id, is_first = false})
        end)
    else
        if self.curMap2D ~= nil then
            self:closeMap()
...
function M:backToHigherMap()
    if self.curMap2D ~= nil then
        ...
        local grandMotherMapId = 0
        if self.map_table[motherMapId] then
            grandMotherMapId = self.map_table[motherMapId].mother_map_id
        end
        if grandMotherMapId ~= 0 and motherMapId ~= 0 and motherMapId ~= self.curMap2D.map_id then
    ...
    if self.curMap2D ~= nil then
        ...
        if mother_map_id ~= self.curMap2D.map_id then
            self:changeScene(mother_map_id, function()
                ...
                self.curMap2D = require("UI.ThreeHeroesFiveGallants.ThreeHeroesFiveGallantsMap2D.Map2D").new(self.m_control, {Map2DControl = self, map_id = mother_map_id, is_first = false})
            end)
        else
            self:closeMap()
        end
...
function M:changeScene(map_id, changeFunc)
    if self.changeSceneNode == nil then
        local tab_cls = CustomRequire("UI.ThreeHeroesFiveGallants.ThreeHeroesFiveGallantsMap2D.ChangeSceneNode")
