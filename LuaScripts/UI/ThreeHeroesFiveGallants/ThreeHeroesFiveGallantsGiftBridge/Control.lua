local M = class("ThreeHeroesFiveGallantsGiftBridgeControl",LikeOO.OOControlBase)
...
function M:onHandle(msg , data)
    ...
    elseif msg == "buy_hero_btn" then
        local charge_id = self.m_model.m_hero_skin_data.charge_id
        if charge_id and self.m_model:getClothesGift() == 0 then
...
function M:refreshData()
    ...
    self.m_model:getNetData("active_common_gift_index",{open_id = self.m_model.open_id,vsn = self.m_model.m_version }, netCallback)
end
...
function M:showHeroInfo()
    ...
    self:openView("Pops.HeroLookInfo", {hero_id = self.m_model.m_hero_skin_data.reward[1][2], is_new = false})
end
