local M = class("NationalBeautifulBeforeStoryControl",LikeOO.OOControlBase)
...
function M:requestStory()
    local function netCallback(response)
        if response and response.new_unlock_hero and next(response.new_unlock_hero) then
            ...
            self:updateMsg("unlock_new_chapter", response.new_unlock_hero, "NationalBeautiful.NationalBeautifulXkzDetail")
        end
    ...
    self.m_model:getNetData("raccon_common_event_end", {open_id = self.m_model.open_id,vsn = self.m_model.version,hero_id = self.m_model.hero_index, stage_id = self.m_model.stage_id}, netCallback, nil, true)
end
