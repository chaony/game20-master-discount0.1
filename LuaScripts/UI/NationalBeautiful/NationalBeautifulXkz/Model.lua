local M = class("NationalBeautifulXkzModel", LikeOO.OODataBase)

function M:onCreate()
	M.super.onCreate(self)
	self.open_id = self.m_params.open_id or 410
	self.version = self.m_params.version or 1
	local cfg = ConfigManager:getCfgByName("common_biography_chapter") or {}
	for k,v in pairs(cfg) do
		local data = v 
		for m,n in pairs(data) do 
			local data_ = n
			for i,u in pairs(data_) do
				if u.version_open == 1 then
					self.open_id = tonumber(k)
					self.version = tonumber(m)
					break
				end
			end
		end
	end
	self:getData("raccon_common_index",{open_id = self.open_id,vsn = self.version})
end
...
function M:onEnter()
	...
	self.m_rbc_cfg = ConfigManager:getCfgByName("common_biography_chapter")[self.open_id][self.version] or {}

	self.m_raccon_biography_hero_cfg = ConfigManager:getCfgByName("common_biography_hero")[self.open_id][self.version] or {}
	self.m_raccon_biography_condition_cfg = ConfigManager:getCfgByName("common_raccon_biography_option") or {}
	self.m_rbc_reward_cfg = ConfigManager:getCfgByName("biography_chapter_reward")or {}
...
function M:getEndTs()
	local active = UserDataManager:getActivesDataByOpenId(self.m_params.open_id)
...
function M:getActiveData(open_id)
	...
	local version = self.m_params.version
