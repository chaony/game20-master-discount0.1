local M = class("PredestinedWashHelpPopModel", LikeOO.OODataBase)

function M:onCreate()
	M.super.onCreate(self)
	local gacha_cfg = ConfigManager:getCfgByName("gacha")
	local cfg = gacha_cfg[7]
	local param = {}
	param.open_id = cfg.gift_open_id
	param.vsn = cfg.gift_open_vsn
	self:getData("active_common_gift_index", param)
	self.m_open_id = cfg.gift_open_id
	self.m_version = cfg.gift_open_vsn
