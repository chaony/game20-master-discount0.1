local M = class("UnionPopView",LikeOO.OOPopBase)

M.m_uiName = "Union/UnionPop"
M.m_size_type = 2
...
function M:onEnter()
	...
	self:setTextByLanKey("apply_btn_text", "union_str_1034")
	...
	self:setTextByLanKey("one_apply_btn_text", "union_one_apply_tex")
...
function M:refreshUI()
	...
	if guild_data then
		self:findGameObject("apply_btn"):SetActive(true)
		...
		self:findGameObject("name_bg_img"):SetActive(false)
		self:findGameObject("list_title_img"):SetActive(true)
		...
		local apply_desc = GameUtil:formatInputText(guild_data.apply_desc)
		self:setText("detail_text", apply_desc)
		...
		self:setTextByLanKey("apply_btn_text", guild_data.is_apply and "friend_str_0013" or "union_str_1034")
		...
		self:findGameObject("apply_btn"):SetActive(false)
		self:findGameObject("president_head"):SetActive(false)
		...
		self:findGameObject("line_3_img"):SetActive(false)
		self:findGameObject("name_bg_img"):SetActive(false)
		self:findGameObject("list_title_img"):SetActive(false)
		self:findGameObject("union_icon_bg"):SetActive(false)
		self:findGameObject("union_icon_img"):SetActive(false)
		self:findGameObject("find_input"):SetActive(false)
		self:findGameObject("fresh_btn"):SetActive(false)
		
		local create_btn = self:findGameObject("create_btn")
		local position = create_btn.transform.localPosition
		UIUtil.setLocalPosition(create_btn.transform, 0)
	end
...
function M:updateListScroll()
	...
	self:setObjectVisible("no_text", #data <= 0)
...
function M:listHandle(obj, id, data)
	...
	--lv_text.text = string.format(Language:getTextByKey("new_str_0075"), data.level)
