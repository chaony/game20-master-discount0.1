local M = class("UnionWarSettlementControl",LikeOO.OOControlBase)
...
function M:onHandle(msg , data)
    ...
    elseif msg == "log_btn" then
        ...
        --self:openView("UnionWar.UnionWarLog", {default_tab = 1, union_war_type = 1})
        local function callback(response)
            local function callbackindex(response_index)
                ...
                if guild_id and guild_id > 0 then
                    self:openView("UnionWar.UnionWarLog", {union_war_type = 1, default_tab = 1, active_data = response_index, active_list_data = response.rank_data})
                end
            end
