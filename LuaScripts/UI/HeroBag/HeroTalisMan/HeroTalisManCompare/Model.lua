local M = class("TalisManmentCompareModel", LikeOO.OODataBase)
...
function M:onEnter()
    ...
    self.use_data = self.m_params.use_data
    self.m_talins_id = self.use_data.id or 0
    self.m_all_num = self:getTalisDataById(self.m_talins_id ).number and self:getTalisDataById(self.m_talins_id ).number +1 or 1   --当前符篆总数量
    self.m_open_id = 773
    self.left_value = 0  --左侧分数
    self.right_value = 0 --右侧分数
    self:refreshListData(self.m_params.response)
end

--[[
    仓库数据
]]
function M:refreshListData(response)
    --所以属性数据
    self.m_data = response --所以曹数据数据
    local data_ = self.m_data.seal_character and self.m_data.seal_character[tostring(self.pos)] or  self.m_data[tostring(self.pos)]
    self.left_data = {id = data_.id,attrs = data_.attrs} --左侧数据
    self.right_data ={id = data_.tmp_id,attrs = data_.tmp_attrs} --右侧数据
end
...
function M:getHeroTalinsEfeectNum()
    local curNum = 1
    local allNum = 4
    --local h_data= self:getHeroTalinsManList()
