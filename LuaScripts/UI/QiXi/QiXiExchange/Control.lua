local M = class("QiXiExchangeControl",LikeOO.OOControlBase)
...
function M:onHandle(msg , data)
    ...
    elseif msg == "reward_btn" then
        self.m_model:getNetData("active_common_exchange", {vsn = self.m_model:getVersion(), gift_id = data.gift_id,open_id = self.m_model.open_id, num = data.num}, function(response)
            if response then
