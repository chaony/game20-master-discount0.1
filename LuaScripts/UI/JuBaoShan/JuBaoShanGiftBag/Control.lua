local M = class("JuBaoShanGiftBagControl",LikeOO.OOControlBase)
...
function M:onHandle(msg , data)
    if msg == 99999 then    -- 返回
        self:updateMsg("update_red_point", nil, "JuBaoShan")
...
function M:highGachaVoyageBuyGift(data)
    ...
    if sort == 1 then -- 元宝
        local function netCallback(response)
            ...
            self:updateMsg("refresh_ui", nil, "JuBaoShan")
        end
        local params = {gift_id = cell_data.id}
        self.m_model:getNetData("richman_buy_gift", params, netCallback)
