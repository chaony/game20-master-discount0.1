local M = class("GuildHighWarNewMainYanModel", LikeOO.OODataBase)
...
function M:onCreate()
	...
	self:getData("guild_high_war_index")
end

function M:onEnter()
	--Logger.logError(self.m_data, "----------巅峰--")
	self.guild_high_war = self.m_data
	local ghw_teams = self.guild_high_war and self.guild_high_war.teams or {}
	UserDataManager:setGuildHighWarTeams(ghw_teams)
	self.m_sel_tab_index = 1
	self.chapter_season_list ={{name = "guild_high_war_new_007",type = 4},{name = "guild_high_war_new_008",type= 6},{name = "guild_high_war_new_009",type = 8},{name = "guild_high_war_new_0010",type = 10}}
	self.big_stage_list = {{type = 2,cycle = 1,curIndex = 1},{type = 2,cycle = 2,curIndex = 2},{type = 2,cycle = 3,curIndex = 3},{type = 3,cycle = 1,curIndex = 4},{type = 3,cycle = 2,curIndex = 5}}
	self.m_reward_data = {}
	...
	self.cycle_start_ts = self.guild_high_war.cycle_start_ts -- 开始时间
	...
	self.m_new_reward_data = self:setRewardData()
	self.is_go_game = false --防止连续点击
	self.is_share = self.guild_high_war.is_share or 0
	self:InitData()
end

function M:setRewardData()
	local cfg = ConfigManager:getCfgByName("common")
	local data = cfg[797] and cfg[797].value or {}
	return data
end
...
function M:getCycleAllTs()
	local allTs = self.cycle_end_ts -  self.cycle_start_ts
	return allTs ==0 and 1 or allTs
end
