...
function M:onEnter()
	self.m_pop_log = self.m_params.pop_log or {}
	self.m_logs = self.m_pop_log.logs or {}
	self.m_mvp = self.m_pop_log.mvp or nil
	self.m_log_time = self.m_pop_log.log_time or 0
	self.m_is_good = self.m_mvp and self.m_mvp.is_liked or false
	self.like_num = self.m_mvp and self.m_mvp.like or 0
	self.m_cur_select_id = self.m_logs[1] and self.m_logs[1].city_id or 0
	self:initCityId()
end

function M:updateGoodData(response)
	self.m_mvp.like = response.like
	self.m_is_good = response.is_liked
end
...
function M:initCityId()
	...
	for i = 1, #self.m_logs do
		...
		--if self.m_cur_select_id == nil and i == 1 and count>0 then
