local M = class("GuildHighNewMainControl",LikeOO.OOControlBase)
...
function M:onHandle(msg , data)
    ...
    elseif msg == "update_stage" then
        local function netCallback(response)
            if response then
                self.m_model:UpdateWarData(response.guild_high_war)
                self.m_view:refreshUI()
                self.m_model.is_can_updata = false
            end
        end
        self.m_model:getNetData("gvg_index", nil, netCallback)
        ------------------------------------------------------------------
    ...
    elseif msg == "updateGress" then
        self.m_view:refreshUI()
    elseif msg == "schedule_btn" then
        self:openView("GuildHighWar.GuildHighWarSchedulePop")
    end
end
