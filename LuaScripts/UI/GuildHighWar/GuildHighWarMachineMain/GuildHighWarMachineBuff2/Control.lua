...
function M:onHandle(msg , data)
    ...
    elseif msg == "refresh_data" then
        self.m_model:InitData(data.user_data_page_2)
        ...
        local params = {
            ...
            right_callback= function()
                local function receivetCallback(response)
                    self.m_model:InitData(response.user_data_page_2)
                    self.m_view:refreshUI()
...
function M:clickBtn(msg)
    ...
    self:openView("GuildHighWar.GuildHighWarMachineMain.GuildHighWarMachineLevelPop",{data = data,page_id = 2,talent_point= self.m_model.talent_point})
