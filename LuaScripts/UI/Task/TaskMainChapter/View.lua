---@class TaskMainChapterView:OOPopBase
---@field m_model TaskMainChapterModel
local M = class("TaskMainChapterView",LikeOO.OOPopBase)

M.m_uiName = "Task/TaskMainChapter"
M.m_size_type = 2

function M:onEnter()
	if self.m_model.m_quest_type == 1 then
		self:setTextByLanKey("common_title_text", "new_str_1051")
	elseif self.m_model.m_quest_type == 44 then
		self:setTextByLanKey("common_title_text", "new_str_0659")
	elseif self.m_model.m_quest_type == 104 then
		self:setTextByLanKey("common_title_text", "jubaoShan_str_012")
	elseif self.m_model.isle_flag then
		self:setTextByLanKey("common_title_text", "new_str_1120")
		self:setTextByLanKey("countdown_tip_text", "new_str_1128")
	else
		self:setTextByLanKey("common_title_text", "new_str_0648")
	end

	self:setObjectVisible("countdown",self.m_model.isle_flag)
	...
	if show_point_cfg then
		...
		if #drop > 0 then
			reward_item_node:SetActive(true)
			...
			reward_item_node:SetActive(false)
			self:setTextByLanKey("tips_text","new_str_0720")
		end
	else
		reward_item_node:SetActive(false)
		self:setTextByLanKey("tips_text","new_str_0720")
	end
	self:setTextByLanKey("auto_get_btn_text","mail_str_0017")
...
function M:refreshUI()
	...
	self:setObjectVisible("auto_get_btn", self.m_model:getGetFlag())
end

function M:updateTime()
	if self.m_model.m_end_ts ~= nil then
		local time_end = self.m_model.m_end_ts - UserDataManager:getServerTime()
		self:setText("countdown__text",GameUtil:formatTimeBySecond(time_end))
		--if time_end <= 0 then
...
function M:updateScrollViewCell(index, cell_object, cell_data)
	...
	-- 名字
	UIUtil.setTextByLanKey(transform, "name_text", cfg.name)
	-- 进度条
	...
	if status == 0 then--前往
		...
		if _G.next(go_type) then
			...
			UIUtil.setImg(transform, "a_bhzf_paihang_di", "active_ui", "goto_btn")
	...
	GameUtil:createRewards(reward_node, drop, true, true, nil, 0.8)
