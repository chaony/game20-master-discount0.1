--- 周任务
local M = class("TaskWeekNode",LikeOO.OOUIbase)
...
function M:onEnter()
    ...
    self.onceOpen = true
    local tips_str = self.m_model:getRewardUpgradeTipsStr(399)
...
function M:refreshUI()
    local time = self.m_model:getWeeklyRemainingTime()
    ...
    EventDispatcher:registerTimeEvent("TaskWeekNodeTime", tick, 1, time)
    self:setTimeText()
...
function M:refreshQuestsScore()
    ...
    local show_data, cur_score = self.m_model:getWeeklyQuestsScoreData()
    ...
    for i=1,box_num do
        ...
        local function btns(trans,params)
            if data.status == 2 then -- 可领取
                self:updateMsg("weekly_box_reward", {click_transform = trans, data = data})
            else
...
function M:refreshTaskLoopScroll()
	...
	local data = self.m_model:getWeeklyQuestsData()
	if self.m_loop_scroll_view == nil then
		...
		local params = {
			...
			click_func = function(index, cell_object, cell_data, click_object, click_name)
                ...
                if status ~= -1 then
                    self:updateMsg(status == 2 and "weekly_reward" or "goto_btn", cell_data)
...
function M:destroy()
    self:killAnim()
    EventDispatcher:unRegisterEvent("TaskWeekNodeTime")
