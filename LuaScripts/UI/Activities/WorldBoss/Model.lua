local M = class("WorldBossPopModel", LikeOO.OODataBase)

function M:onCreate()
	M.super.onCreate(self)
	self:getData("world_boss_index")
end

function M:onEnter()
	-- Logger.log(self.m_data,"data === ")
	--战斗id
	self.m_boss_id = self.m_data.boss_id;
	if self.m_params.boss_id ~= nil then
		self.m_boss_id = self.m_params.boss_id;
	end
	if not self.m_boss_id then
		self.m_boss_id = 1
	end
	self.m_mode = self.m_params.mode;
	Logger.log(" World_Boss boss_id = "..self.m_boss_id.." 战斗类型 "..tostring(self.m_mode) )
	self:updateDanData()
...
function M:getLeftTimes()
	...
	--local world_cfg = world_boss[self.m_data.world_boss_id]
	--return (self.m_data.buy_times + 1) * world_cfg.times - self.m_data.battle_times
...
function M:getNewBossRewardCfg()
	...
	if world_boss_reward_cfg and next(world_boss_reward_cfg) and world_boss_reward_cfg[self.m_boss_id] then
		local reward_data = world_boss_reward_cfg[self.m_boss_id][self.m_boss_hp_cid] or {}
