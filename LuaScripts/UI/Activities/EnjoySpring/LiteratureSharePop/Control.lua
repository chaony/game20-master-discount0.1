local M = class("LiteratureSharePopControl",LikeOO.OOControlBase)
...
function M:onHandle(msg, data)
    ...
    elseif msg == "out_btn" then
        self:openView("Activities.EnjoySpring.LiteratureRankSharePicture")
    end
end
...
function M:getChatData()
    ...
    ext_params.star = self.m_model.m_params.star
    local send_data = {
        ...
        msg = Language:getTextByKey("enjoySpring_str_0035"),
        event = 1,
...
function M:onRefreshChatInfo()
    ...
    if msgs and msgs.event and msgs.event == 1 and curPlayerId == msgs.uid then
        GameUtil:lookInfoTips(self, {msg = Language:getTextByKey("enjoySpring_str_0037"), delay_close = 2})
    end
end
