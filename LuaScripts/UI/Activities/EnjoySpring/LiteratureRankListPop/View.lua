local M = class("LiteratureRankListPopView",LikeOO.OOPopBase)

M.m_uiName = "Activities/EnjoySpring/LiteratureRankListPop"
M.m_size_type = 2
	...
	if luaBehaviour then
		if next(cell_data) ~= nil then
			...
			if my and my == true then
				if self.m_model.m_data.rank and self.m_model.m_data.rank > 0 then
				...
				local score = self.m_model.m_data.score or 0
				LuaBehaviourUtil.setTextByLanKey(luaBehaviour, "race_score_text", score)
				local head_node = luaBehaviour:FindGameObject("head_node")
