local M = class("LiteratureGroupRankListPopView",LikeOO.OOPopBase)

M.m_uiName = "Activities/EnjoySpring/LiteratureGroupRankListPop"
M.m_size_type = 2
...
function M:onEnter()
	...
	self:setTextByLanKey("out_force_btn_text", "enjoySpring_str_0040")
	self:setTextByLanKey("tips_text", "four_force_war_0004")
	self:setObjectVisible("out_force_btn", false)
	self:refreshUI()
end
...
function M:updateTeam(cell_object, cell_data, my)
	...
	if luaBehaviour then
		if next(cell_data) ~= nil then
			...
			local force = self.m_model:getEnjoySpringForceCfg(cell_data.force)
			LuaBehaviourUtil.setTextByLanKey(luaBehaviour, "name_text", force.name)
			--LuaBehaviourUtil.setTextByLanKey(luaBehaviour, "level_text", force.name)
			...
			if my and my == true then
				...
				local force = self.m_model:getEnjoySpringForceCfg(self.m_model.m_force)
				LuaBehaviourUtil.setTextByLanKey(luaBehaviour, "name_text", force.name)
