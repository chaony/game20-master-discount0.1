local M = class("FourForceWarGroupSelectPopControl",LikeOO.OOControlBase)
...
function M:requestJoin()
    local function joinCallback(response)
        local function rewardCallback()
            self:updateMsg("update_data", {msg_source = "literature_group_select"}, "Activities.FourForceWar")
            self:closeView("Activities.FourForceWar.FourForceWar")
