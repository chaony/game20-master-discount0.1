local M = class("FourForceWarTaskView",LikeOO.OOPopBase)

M.m_uiName = "Activities/FourForceWar/FourForceWarTask"
M.m_size_type = 2
...
function M:onEnter()
	...
	for k,v in pairs(__TAB_BTN_NODE_1) do
		if k == 1 then
			...
			self:setText(v.btn_text, string.cutTextForString(Language:getTextByKey("four_force_war_0003")))
		end
...
function M:refreshUI()
	...
	for k,v in pairs(__TAB_BTN_NODE_2) do
		if k == self.m_model.m_task_tab then
			self:setTextColor(v.btn_text, GlobalConfig.COMMON_COLLOR.COMMON_27)
		else
			self:setTextColor(v.btn_text, GlobalConfig.COMMON_COLLOR.COMMON_25)
		end
	end
