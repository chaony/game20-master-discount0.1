---@class FourForceWarTaskControl:OOControlBase
---@field m_model FourForceWarTaskModel
local M = class("FourForceWarTaskControl",LikeOO.OOControlBase)
...
function M:onHandle(msg, data)
        ...
        if self.m_model:getActStatus() == 1 then
            if self.m_model.m_task_tab == 1 then
                ...
                if task_cfg then
                    ...
                    self:openView("Activities.FourForceWar.FourForceWarSharePop",params)
                end
            end
        else
    ...
    elseif msg == "show_rank_list_btn" then -- 查看排行榜
        ...
        if self.m_model.m_rank_tab == 1 then
            ...
            self:openView("Activities.FourForceWar.FourForceWarRankListPop", params)
        else
            params.data = self.m_model.m_data.force_rank
            params.force = self.m_model.m_data.force
            self:openView("Activities.FourForceWar.FourForceWarGroupRankListPop", params)
        end
    elseif msg == "help_btn" then
        local content = Language:getTextByKey("tid#enjoy_spring2_Des")
    ...
    elseif msg == "gift_bag_btn" then
        local params = {}
        params.openId = self.m_model.open_id
        params.version = self.m_model.m_data.version
        self:openView("FlowerFestival.FlowerExchange", params)
    end
end
