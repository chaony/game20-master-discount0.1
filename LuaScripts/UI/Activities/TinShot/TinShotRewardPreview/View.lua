local M = class("TinShotRewardPreviewView", LikeOO.OOPopBase)

M.m_uiName = "Activities/TinShot/TinShotRewardPreview"
M.m_size_type = 2
...
function M:createLoopScroll()
    local data = {}
    if self.m_model.m_is_tongyong == true then
        data = self.m_model:getTongYongBigReward()
    else
        data = self.m_model:getBigReward()
    end
    if self.m_scroll_view == nil then
