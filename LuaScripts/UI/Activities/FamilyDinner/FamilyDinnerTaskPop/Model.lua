local M = class("FamilyDinnerTaskPopModel", LikeOO.OODataBase)
...
function M:onEnter()
	...
	self.m_key = 0 -- 期数
	self.m_guild_score = self.m_params.guild_score or 0 --帮会积分
	self.m_own_score = self.m_params.own_score or 0 --个人积分
	...
	local cur_cfg = vsn_tab[self.m_key] --本期数据
	self.m_value_des = cur_cfg.des or ""
	self.m_value_own = cur_cfg.own_des or ""
...
function M:getTaskData(id, score, ownscore)
	-- 0 进行中
	-- 1 可领奖
	-- 2 已完成
	...
	if self.m_guild_score >= score and self.m_own_score >= ownscore then
		return 1
	end
	return 0
end
