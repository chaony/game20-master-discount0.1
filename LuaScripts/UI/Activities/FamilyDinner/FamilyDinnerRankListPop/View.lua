local M = class("FamilyDinnerRankListPopView",LikeOO.OOPopBase)

M.m_uiName = "Activities/FamilyDinner/FamilyDinnerRankListPop"
M.m_size_type = 2
...
local __TAB_BTN_NODE = { 
	...
	{btn_key = "tog_3", text_key = "tog_3_text", show_text = "qi_men_dun_jia_str_008", node = "reward_node" }, -- 奖励
	{btn_key = "tog_4", text_key = "tog_4_text", show_text = "new_str_1097", node = "reward_node2" }, -- 奖励
}
...
function M:onEnter()
	...
	self:setTextByLanKey("score_label_text", "new_str_1098")
	self:setTextByLanKey("rank_label_text1", "new_str_0114")
	self:setTextByLanKey("rank_label_text2", "new_str_0114")
	self:setTextByLanKey("rank_label_text3", "new_str_0374")
	self:setTextByLanKey("union_name_text", "union_name_tex")
	self:setTextByLanKey("server_text", "qufu_tex")
	self:setTextByLanKey("union_progress_label_text", "total_world_rank_title_1")
	self:setTextByLanKey("own_rank_title_text", "flower_text_0069")
	self:setTextByLanKey("own_rank_title_text2", "new_str_0077")
	self:setTextByLanKey("player_label_text", "player_bal_tex")
	self:setTextByLanKey("union_label_text", "union_label_tex")
	

	self:setTextByLanKey("text", Language:getTextByKey("flower_text_0069")..":")
	self:refreshUI()
end
...
function M:refreshUI()
	...
	elseif self.m_model.m_sel_tab_index == 4 then
		self:createRewardLoopScroll2()
	end
end
...
function M:updateInsideTeam(cell_object, cell_data, my)
	...
	if luaBehaviour then
		if next(cell_data) ~= nil then
			...
			LuaBehaviourUtil.setObjectVisible(luaBehaviour, "rank_text", rank ~= 0)
			LuaBehaviourUtil.setObjectVisible(luaBehaviour, "none_rank_text", rank == 0)
...
function M:updateTeam(cell_object, cell_data, my)
	...
	if luaBehaviour then
		if next(cell_data) ~= nil then
			...
			local server_name = cell_data.user.server_name--UserDataManager.server_data:getServerNameById(cell_data.user.server)
...
function M:createRewardLoopScroll2()
	local data = self.m_model.ranks_rewards2
	if self.m_scroll_view3 == nil then
		local loopscroll = self:findGameObject("reward_loopscroll2")
		local params = {
			show_data = data,
			loop_scroll_object = loopscroll,
			update_cell = function(index, cell_obj, cell_data)
				self:updateRewardItem2(cell_obj, index, cell_data)
			end,
			click_func = function(index, cell_object, cell_data, click_object, click_name)
...
function M:updateRewardItem2(obj, index, data)
	local luaBehaviour = UIUtil.findLuaBehaviour(obj)
	if luaBehaviour then
		LuaBehaviourUtil.setObjectVisible(luaBehaviour, "top_three_rank_img", index <= 3)
		LuaBehaviourUtil.setObjectVisible(luaBehaviour, "rank_bg", index > 3)
