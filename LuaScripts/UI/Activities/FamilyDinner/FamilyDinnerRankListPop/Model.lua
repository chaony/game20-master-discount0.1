local M = class("FamilyDinnerRankListPopModel", LikeOO.OODataBase)
...
function M:onEnter()
	...
	self.ranks_rewards2 = self:getRankRewards2()
end
...
function M:getRankRewards2()
	local dinner_rank_tab = ConfigManager:getCfgByName("dinner_rank_personal");
	if next(dinner_rank_tab) == nil then
		return {}
	end
	local vsn_reward_tab = dinner_rank_tab[self.m_version]
