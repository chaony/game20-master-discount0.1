...
-- date:  2022/7/12
--==================================
local M = class("FamilyDinnerPopView",LikeOO.OOPopBase)

M.m_uiName = "Activities/FamilyDinner/FamilyDinnerPop"
M.m_size_type = 2
...
function M:updateScrollViewCell(index, cell_object, cell_data)
	...
	if refresh == 0 then -- 无限制  --先改为每日刷新 默认0 因为现在不需要控制
	...
	LuaBehaviourUtil.setTextByLanKey(luaBehaviour,"score_text", "new_str_1082",cfg.score) --积分
	...
	if sort == 1 then -- 元宝
		LuaBehaviourUtil.setTextByLanKey(luaBehaviour,"cost_num_text", tostring(cfg.price or 999))
	elseif sort == 2 then -- 充值金额
		LuaBehaviourUtil.setTextByLanKey(luaBehaviour,"buy_btn_text", GameUtil:getMoneyTypeNum(cfg.price or 999))
	else
