---@class DeliciousFeastGiftBagModel: OODataBase
local M = class("DragonBoatGiftBagModel", LikeOO.OODataBase)

function M:onCreate()
    M.super.onCreate(self)
    self.m_open_id = self.m_params.openId or 354
    self.m_version = self.m_params.versionId or 1
...
function M:getSpineName()
    if self.m_params.spineName == nil then
        self.m_params.spineName = { meituan = "MeiTuan_SkeletonData", xian = "hero_0101_yongzhuang_SkeletonData" }
    end
    return self.m_params.spineName
end

function M:checkIsOfficial()
    if self.m_params.isOfficial == nil then
        self.m_params.isOfficial = false
        if SDKUtil.is_gmsdk then
            local bundleId = SDKUtil.sdk_params.applicationId or ""
            if bundleId == "com.hermes.wl" then
                self.m_params.isOfficial = true
            end
        end
    end
