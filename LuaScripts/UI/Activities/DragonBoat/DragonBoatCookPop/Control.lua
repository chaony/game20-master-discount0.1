...
local M = class("DragonBoatCookPopControl", LikeOO.OOControlBase)
...
function M:cookFood(num)
    local function callback(response)
        if response.reward then
            self:updateMsg("refreshNum", nil, "Activities.DragonBoat.DragonBoatCook")
            self:updateMsg("refreshNum", response.total_cook_times, "Activities.DragonBoat.DragonBoatCookThree")
            self:closeView()
