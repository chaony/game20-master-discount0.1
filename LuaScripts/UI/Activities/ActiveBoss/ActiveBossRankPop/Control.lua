local M = class("ActiveBossRankPopControl",LikeOO.OOControlBase)
...
function M:onHandle(msg , data)
    ...
    elseif msg == "good" then    --点赞
        ...
        self.m_model:getNetData("common_world_boss_like", { target_uid = data.uid, open_id = self.m_model.m_open_id, vsn = self.m_model.m_version }, callback)
    elseif msg == "box_reward" then --领奖
        ...
        self.m_model:getNetData("common_world_boss_recv_reward", { rank = data.data, open_id = self.m_model.m_open_id, vsn = self.m_model.m_version }, callback)
    elseif msg == "auto_btn" then
        ...
        self.m_model:getNetData("common_world_boss_auto_recv_reward", { open_id = self.m_model.m_open_id, vsn = self.m_model.m_version }, callback)
    ...
    elseif msg == "statistics_btn" then
        if data and data.battle_id then
            ...
            self:openView("Pops.BattleStatistics", {battle_id = data.battle_id, round = 1, mode = GlobalConfig.BATTLE_MODE.ACTIVE_BOSS})
        end
    end
end
