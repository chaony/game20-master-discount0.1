---@class PrestigeGetPopView : OOPopBase
local M = class("PrestigeGetPopView", LikeOO.OOPopBase)

local __MINI_SLOT_SIDE_LENGTH = 25
local __ROTATE_TIME_THRESHOLD = 0.15

M.m_uiName = "Prestige/PrestigeGetPop"
M.m_size_type = 2
...
function M:onEnter()
    ...
    --self:initView()
    self:refreshView()
    self:updateMsg("end_new_data",nil,"Prestige.PrestigeGetPop")
end
...
function M:updateBlockInfo(cell_index, cell_object, cell_data)
    ...
    --LuaBehaviourUtil.setTextByLanKey(luaBehaviour, "equipment_text", )
    -- 星级
...
function M:OnClickGridItem(cell_index, cell_object, cell_data)
    ...
    local res = not obj.activeSelf
    LuaBehaviourUtil.setObjectVisible(luaBehaviour, "selected_mask", res)

    local flagObj = self:findGameObject("selected_img")
    if res == false and flagObj.activeSelf then
        self:setObjectVisible("selected_img", false)
        self.m_control.is_all_selected = false
    end

    self.m_control:onSelectedSingleGrid(cell_data.id)
end
