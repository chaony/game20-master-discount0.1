local M = class("HuntTreasuresAreaInfoPop",LikeOO.OOControlBase)

function M:onEnter()
    self.m_guide_file_name = "UI.HuntTreasures.HuntTreasuresAreaInfoPop.Guide"
    self:updateTime()
...
function M:onHandle(msg , data)
    ...
    elseif msg == "click_hero" or msg == "team_edit_btn" then
        ...
        if self_uid == self.m_model.m_data.def_uid then
            ...
            self:openView("Formation",{ races = races, mode = GlobalConfig.BATTLE_MODE.MINING_DEFENSE, formation_index = team_id})
        end
        ...
        elseif self.m_model:getOccupyOverTimeTips() ~= "" and self.m_model.m_robot_id == 0 then
            ...
            local function toBattle()
                ...
                self:openView("Formation",{region_id = region_id,
                                           def_data = def_data,
                                           races = races,
                                           mode = GlobalConfig.BATTLE_MODE.MINING,
...
function M:requestRecall()
    local function realRequest()
        ...
        self.m_model:getNetData("mining_recall_team", { team_id = self.m_model.m_data.team_id }, callfunc)
    end
...
function M:showBuyWindow()
    ...
    local params =
    {
        ...
        --通知的类名
        className = "HuntTreasures.HuntTreasuresAreaInfoPop",
        --最大购买次数
        ...
        clickBuy = function( num )
            local function netCallback(response)
                ...
                self:updateMsg("update_buy_plunder", { buy_plunder = response.buy_plunder },"HuntTreasures")
...
function M:freshNetData()
    ...
    self.m_model:getNetData("mining_mine_detail",{mine_oid = self.m_model.m_mine_oid}, receivetCallback)
end
