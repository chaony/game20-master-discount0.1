local M = class("HuntTreasuresAreaInfoPopView",LikeOO.OOPopBase)

M.m_uiName = "HuntTreasures/HuntTreasuresAreaInfoPop"
M.m_size_type = 2
...
function M:onEnter()
    ...
    self:setTextByLanKey("def_text", "hunt_treasure_def_text")
    self:refreshUI()
...
function M:updateTime()
    if self.m_model:getRewardReceiveTime() > 0 then
        ...
        local tim = GameUtil:formatTimeBySecond(self.m_model:getOccupyTime(), 999)
...
function M:refreshOwnerInfoNode()
    ...
    local server_name = self.m_model.m_user and self.m_model.m_user.server_name or ""
