local M = class("HuntTreasuresView",LikeOO.OOPopBase)

M.m_uiName = "HuntTreasures/HuntTreasures"
M.m_iphoneXAdapter = true
local __title_img_tab = {"tid#mining_regin1_01", "tid#mining_regin1_02", "tid#mining_regin1_03", "tid#mining_regin1_04"}
function M:onEnter()
    ...
    self:setTextByLanKey("close_title_text", "hunt_treasure_str_028")
    ...
    self:setTextByLanKey("shop_btn_text", "hunt_treasure_str_058")
    ...
    local tab_cls = CustomRequire("UI.HuntTreasures.HuntTreasuresTotalAreaNode")
    self.m_area_node = tab_cls.new(self.m_control, {parent = self.m_content_panel})
    self.m_area_node:updateData(self.m_model.m_data)
    self:refreshUI()
    local log_flag = UserDataManager:getRedDotByKey("mining_blog") --有新战报
    if log_flag == 1 then
...
function M:refreshUI()
    ...
    local race_tab = self.m_model.m_mining_region_config[self.m_model.m_region_id].race
    ...
    self:setTextByLanKey("union_add_text", "hunt_treasure_str_015", self.m_model:getGuildAdd())
    ...
    local race_cft = GlobalConfig.TYPE_HERO_RACE[race_tab[2]]
    self:setImg(race_cft.race_icon, ResourceUtil:getLanAtlas(), "icon_img_2")
    self:findImage("icon_img_1"):SetNativeSize()
    self:findImage("icon_img_2"):SetNativeSize()
...
function M:refreshRedPoint()
    local reward_flag = UserDataManager:getRedDotByKey("mining_reward") --有奖励可用
    local team_flag = UserDataManager:getRedDotByKey("mining_team") --有编队可用
    local log_flag = UserDataManager:getRedDotByKey("mining_blog") --有新战报
    local shop_flag = UserDataManager:getRedDotByKey("mining_shop_goods") --苗疆商店
    self:setObjectVisible("team_red_point_img", reward_flag == 1 or team_flag == 1)
    self:setObjectVisible("log_red_point_img", log_flag == 1)
    self:setObjectVisible("shop_red_point_img", shop_flag == 1)
end
