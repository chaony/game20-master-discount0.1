local M = class("ActiveCurrentRankListPopModel", LikeOO.OODataBase)
...
function M:onCreate()
	...
	if self.m_open_type == "grow" then
		self:getData("hero_event_get_combat_rank", {version = self.m_vsn, start = 1, stop = 50})
	else
		self:getData()
	end
end
...
function M:getRankRewards()
	local base_reward_tab = {}-- ConfigManager:getCfgByName("hero_event_ranking")
	if self.m_open_type == "grow" then
		base_reward_tab = ConfigManager:getCfgByName("hero_event_rank") or {}
	else
		base_reward_tab = ConfigManager:getCfgByName("hero_event_ranking") or {}
	end
