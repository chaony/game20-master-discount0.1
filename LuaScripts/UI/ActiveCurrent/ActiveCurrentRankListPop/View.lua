local M = class("ActiveCurrentRankListPopView",LikeOO.OOPopBase)

M.m_uiName = "ActiveCurrent/ActiveCurrentRankListPop"
M.m_size_type = 2
...
function M:onEnter()
	...
	if self.m_model.m_open_type == "grow" then
		self:setTextByLanKey("score_label_text", "new_str_0309")
		self:setTextByLanKey("common_title_text", "new_str_0114")
	else
		self:setTextByLanKey("score_label_text", "evil_shadow_str_009")
		self:setTextByLanKey("common_title_text", "evil_shadow_str_008")
	end
	self:refreshUI()
end
...
function M:refreshUI()
	if self.m_model.m_sel_tab_index == 1 then
		...
		self:setObjectVisible("own_content", self.m_model.m_open_type ~= "grow")
		if self.m_model.m_open_type ~= "grow" then
			local own_info = self:findGameObject("own_info")
			local rank_data = self.m_model:myRanks()
			if rank_data.rank == nil or rank_data.score == nil then
				own_info:SetActive(false)
			end
			self:updateTeam(own_info, rank_data, true)
		end
	else
...
function M:updateTeam(cell_object, cell_data, my)
	...
	if luaBehaviour then
		if next(cell_data) ~= nil then
			...
			--if self.m_model.m_open_type == "grow" then
			--	LuaBehaviourUtil.setTextByLanKey(luaBehaviour, "race_score_text", GameUtil:formatValueToString(cell_data.user.full_combat))
			--else
			--	LuaBehaviourUtil.setTextByLanKey(luaBehaviour, "race_score_text", GameUtil:formatValueToString(cell_data.score))
			--end
		else
    ...
    if self.m_scroll_view2 == nil then
        ...
        local params = {
            ...
            update_cell = function(index, cell_obj, cell_data)
                self:updateRewardItem(cell_obj, index, cell_data, allCount)
            end,
            click_func = function(index, cell_object, cell_data, click_object, click_name)
...
function M:updateRewardItem(obj, index, data, allCount)
    ...
    if luaBehaviour then
        ...
        if index <= 3 then
			...
			if self.m_model.m_open_type == "grow" then
				if index == allCount then
					rank_str = Language:getTextByKey("kingsoft_text_0049", data.rank[1])
				else
					rank_str = data.rank[1] .. (data.rank[2] and "-" .. data.rank[2] or "")
				end
			else
				if data.id > 3 then
					rank_str = data.last_id .."-"..data.id
				end
			end
		...
		local reward_data = {} 
		if self.m_model.m_open_type == "grow" then
			reward_data = data.rank_rewards
		else
			reward_data = data.daily_rewards
		end
        GameUtil:createRewards(reward_node.transform, reward_data, true, true)
    end    
end
