local M = class("GambleRankPopView",LikeOO.OOPopBase)

M.m_uiName = "Gamble/GambleRank"
M.m_size_type = 2
...
function M:onEnter()
	...
	self:setTextByLanKey("score_label_text", "gamble_text_010")
	self:setTextByLanKey("common_title_text", "guild_high_war_new_0028")
	self:refreshUI()
end
...
function M:pullRefreshListOffset()
	local now_offsety = self.m_scroll_view.m_scroll_rect.viewport.rect.height - self.m_scroll_view.m_scroll_rect.content.rect.height
	local position = (self.last_offsety - now_offsety) / self.m_scroll_view.m_scroll_rect.content.rect.height
