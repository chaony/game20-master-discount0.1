...
function M:getNewBossRewardCfg()
    if self.m_model.m_mode == GlobalConfig.BATTLE_MODE.ACTIVE_BOSS then
        ...
        local world_boss_item = world_boss_reward_cfg[scene_model.m_data.m_open_id or 327] or {}
        local version_item = world_boss_item[scene_model.m_data.m_version or 1] or {}
        local reward_cfg = version_item[self.m_boss_hp_cid] or {}
        return reward_cfg.reward_lost_hp or {}
    end
    return {}
end
...
function M:sliderAnim()
    ...
    local world_cfg = world_boss_item[scene_model.m_data.m_version or 1] or {}
    local _, _, hp_value = self:getNextRewardDamage()
    
    self.m_silder_fill.color = self.bossHpColor[4]

    local max_damage = self.m_model.m_data.last_damage or 0
