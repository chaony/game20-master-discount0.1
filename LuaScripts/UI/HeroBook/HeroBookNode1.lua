...
local M = class("HeroBookNode1",LikeOO.OOUIbase)
--金
M.m_uiName = "HeroBook/HeroBookNode1"
...
function M:updateHeroUIInfo()
	...
	for i,v in pairs(heros) do
		...
		if not IsNull(spine_obj) and not IsNull(btn_obj) then
			...
			local flag = self.m_model:IsHavePrestigeRed(v.id,v.evo) or table.indexof(hero_id_list, v.id)
			if red_point_img and flag  then
