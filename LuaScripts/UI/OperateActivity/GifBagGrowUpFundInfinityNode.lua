...
--成长基金
M.m_uiName = "OperateActivity/GifBagGrowUpFundInfinityNode"
...
function M:refreshUI()
    ...
    local fund_cfg = self.m_model:get_growth_fund(self.m_open_id) or {}

    self:setText("name_text", fund_cfg.fund_name or "")
    self:setText("info_text", fund_cfg.fund_info or "")
    self:setText("des_1_text", fund_cfg.fund_des1 or "")
    self:setText("des_2_text", fund_cfg.fund_des2 or "")
    self:setText("title_name_3", fund_cfg.fund_rewarddes or "")
   
    
    self:setTextByLanKey("buy_fund_text", GameUtil:getMoneyTypeNum(fund_cfg.price or 0))
    self:createLoopScroll()
    self:setObjectVisible("buy_fund_btn", self.m_model.m_fund_data.fund_status_new[tostring(self.m_open_id)] == nil) --nil 没买，1 已购买
    self:setObjectVisible("get_img", self.m_model.m_fund_data.fund_status_new[tostring(self.m_open_id)] == nil)
...
function M:createLoopScroll()
    self.m_gift_tab = {}
    local data = self.m_model:get_linshi_fund(self.m_open_id)
    if self.m_scroll_view == nil then
        ...
        local params ={
            ...
            click_func = function(index, cell_object, cell_data, click_object, click_name)
                ...
                if can_get > 0 then
                    ...
                    if free_get > 0 and pay_get == 0 and self.m_model.m_fund_data.fund_status_new[tostring(self.m_open_id)] == nil then
                        local params = {
                            text = Language:getTextByKey("gf_str_0113"),
                            tow_close_btn = true,
                            on_ok_call = function ()
                                local fund_cfg = self.m_model:get_growth_fund(self.m_open_id)
                    ...
                    self:updateMsg("get_fund", {reward_id = cell_data.id, open_id = self.m_open_id} )
                end
			end
        }
...
function M:updateCell(index, obj, cfg)
    ...
    if luaBehaviour then
        LuaBehaviourUtil.setText(luaBehaviour, "cell_title", cfg.name)
        LuaBehaviourUtil.setText(luaBehaviour, "cell_title2", string.format(cfg.name2, cfg.target_value))
        ...
        LuaBehaviourUtil.setTextByLanKey(luaBehaviour, "cell_title (1)", "gf_str_0506")
        if can_get > 0 then
            ...
            if self.m_model.m_fund_data.fund_status_new[tostring(self.m_open_id)] ~= nil and pay_get == 0 then
            ...
            self:setLock(pay_obj, self.m_model.m_fund_data.fund_status_new[tostring(self.m_open_id)] == nil)
        else
            buy_img.material = self.m_gray_img.material
            buy_text.text = Language:getTextByKey("new_str_0056")
...
function M:getIndex()
    local fund_table = self.m_model:get_linshi_fund(self.m_open_id)
    for i = 1, #fund_table do
        ...
        if data then
            ...
            local pay_get = tonumber(GameUtil:BitAnd(data.status,4)) or 0 --已领（付费
            if can_get > 0 then
                if self.m_model.m_fund_data.fund_status_new[tostring(self.m_open_id)] == nil then
                    if free_get == 0 then
                        return i
                    end
                else
