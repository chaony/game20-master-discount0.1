---@class SeasonNode3:OOUIbase
---@field m_model SeasonPreviewModel
local M = class("SeasonNode3",LikeOO.OOUIbase)

M.m_uiName = "SeasonPreview/season_3"
...
local jump_icon_tab = {"a_dssj_qimendunjia","a_dssj_tianminghuaxing","a_dssj_xiongshoutaiyin", "a_dssj_qimenyiwu"}
function M:updateFuncDesNode(func_name)
    ...
    if self.m_model.m_is_show_func_node then
        ...
        local img = jump_icon_tab[btn_id - 9] or "a_dssj_tianminghuaxing"
...
function M:setSpine()
    ...
    local max_num=table.nums(hero_id_tab)
    for i = 1, max_num do
        local hk_obj = self:findGameObject("hero_sk" .. i)
        local c_id = hero_id_tab[i]
