---@class SeasonNode2:OOUIbase
---@field m_model SeasonPreviewModel
local M = class("SeasonNode2",LikeOO.OOUIbase)

M.m_uiName = "SeasonPreview/season_2"
...
function M:initUi( ui_data)
    ...
    self:onBtnClick("hero_detail" .. 2, "hero_detail",  2)
...
--local jump_icon_tab = {"a_saiji2_wuxingliansai","a_saiji2_miaojiang","a_saiji2_xinpianzhang", "a_saiji2_kongqueling"}
local jump_icon_tab = {[25]="a_dssj_qimendunjia",[6]="a_dssj_tianminghuaxing",[11]="a_dssj_qimenyiwu", [9]="a_saiji2_wuxingliansai"}
function M:updateFuncDesNode(func_name)
    ...
    if self.m_model.m_is_show_func_node then
        ...
        local img = jump_icon_tab[btn_id] or ""
        
        GameUtil:updateResourcesImg(self.m_jump_icon, "Texture/season_preview/" .. img)
        self.m_jump_icon:SetNativeSize()
        if btn_id == 9 then
            GameUtil:updateResourcesImg(self.m_func_bg_img, "Texture/season_preview/a_saiji2_shenbing")
            self:setImg("a_saiji2_mingzidi2","mystic_ui", "func_name_img")
        else
            GameUtil:updateResourcesImg(self.m_func_bg_img, "Texture/season_preview/a_saiji2_caihongbg")
            self:setImg("a_saiji2_mingzidi","mystic_ui", "func_name_img" )
        end
        self.m_jump_icon:SetNativeSize()
        self.m_func_name_img:SetNativeSize()
...
function M:updateHeroInfo()
    ...
    for i = 1,2 do
        ...
        if i == 1 then
            local temps = Language:getTextByKey(hero_class)
            local names = string.split(temps, "·")
            if names and type(names) == "table" then
                for j = 1, #names do
                    self:setTextByLanKey("hero_type" .. j .. "_text", names[j])
                end
            end
        else
            self:setTextByLanKey("hero_type" .. 3 .. "_text", hero_class)
        end
    end
end
