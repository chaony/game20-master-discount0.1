local M = class("SeasonNode5",LikeOO.OOUIbase)

M.m_uiName = "SeasonPreview/season_5"
...
function M:onEnter()
    ...
    self:setTextByLanKey("road_title_btn_text", "new_str_1088")
...
function M:onBtnClick( btn_name, msg, params)
    ...
    Logger.logWarningAlways(btn_name, "btn_name===============")
    UIUtil.setButtonClick(btn.transform, btnClick, params)
end

local jump_icon_tab = {"a_sj4_tmxx","a_sj4_sjjm","a_sj4_hslj", "a_sj4_wdsl"}
function M:updateFuncDesNode(func_name)
    ...
    if self.m_model.m_is_show_func_node then
        ...
        self:setText("func_name_text", "")
        local img = jump_icon_tab[btn_id - 16] or ""
        
        GameUtil:updateResourcesImg(self.m_jump_icon, "Texture/season_preview/" .. img)
        self.m_jump_icon:SetNativeSize()
        GameUtil:updateResourcesImg(self.m_func_name_img, "Texture/zh_cn/" .. img .. "zi")
...
function M:updateHeroInfo()
    ...
    for i = 1,2 do
        ...
        self:setImg(race,  ResourceUtil:getLanAtlas(), "race"..i)
    end
end
