---@class ArenaPeakControl OOControlBase
---@field m_model ArenaPeakModel
---@field m_view ArenaPeakView
local M = class("ArenaPeakControl",LikeOO.OOControlBase)

function M:onEnter()
    self.m_guide_file_name = "UI.Arena.ArenaPeak.ArenaPeak.Guide"
    ...
    self:setOnceTimer(0.5, function()
        if self.m_model.m_rise_id>=3 then
    ...
    --如果是自动晋级需要显示下晋级效果
    if self.m_model.m_params.promote_flag==1 or (self.m_model.m_params.promote_flag==4 and self.m_model.m_rise_id<5) then
        local param={
            --自动晋级rise_id是晋级后的值，手动晋级时是晋级前的值
            match_type=self.m_model.m_rise_id,
            callback=nil,
            promote_flag=self.m_model.m_params.promote_flag
        }
...
function M:onHandle(msg , data)
    if msg == 99999 then    -- 返回
        ...
        self:updateMsg(99999,nil ,"Arena/ArenaHigher/ArenaHigherDefendTeam")
    ...
    elseif msg == "record_btn" then
        self:openView("Arena.ArenaRace.ArenaRaceLog", {
            free_times = self.m_model.m_data.lave_dare_num,
            is_zf=true,
            ban_num=self.m_model.ban_num,
            team_num=self.m_model.team_num,
            battle_mode=self.m_model.m_battle_mode,
            rise_id=self.m_model.m_data.rise_id,
    ...
    elseif msg == "def_btn" then
        if self.m_model.team_num>1 then
            self:openArenaHigherDefendTeam()
        else
            self:openView("Formation",{mode = GlobalConfig.BATTLE_MODE.ZF_ARENA_DEFENSE,rise_id=self.m_model.m_rise_id,
                                       ban_num=self.m_model.ban_num,
                                       forbidden_hero_ids=self.m_model.heros_ban,
                                       rise_id=self.m_model.m_data.rise_id,
                                       week_rule=self.m_model.m_week_rule,
            })
        end
    elseif msg == "rank_btn" then
        self:openView("Arena.ArenaRace.ArenaRaceRank", { open_type =5,team_num=self.m_model.team_num})
    ...
    elseif msg == "shop_btn" then
        self:openView("Shop", {shop_type = 37, rise_id=self.m_model.m_rise_id})
    elseif msg == "reward_btn" then
        self:openView("Arena.ArenaNormal.ArenaNormalReward", {open_type = 1, data = self.m_model.m_data, cfg_name = "race_arena_reward", content = "tid#arena_explain8"})
    elseif msg == "challenge_btn" then
        ...
        if canChallenge then
            ...
            local daily_times = self.m_model:getCurTimes()
            local cross_season = self.m_model.m_data.cross_season or 1
            local up_nums = self.m_model.m_data.up_nums or 0
            --把种族传过去
            self:openView("Arena.ArenaPeak.ArenaPeakChallenge", { match_type = self.m_model.m_rise_id,
                                                                  ...
                                                                  battle_mode=self.m_model.m_battle_mode,
                                                                  ban_num=self.m_model.ban_num,
                                                                  lave_dare_num=self.m_model.lave_dare_num,
                                                                  heros_ban=self.m_model.m_heros_ban,
                                                                  week_rule=self.m_model.m_week_rule})
        end


    elseif msg == "explain_btn" then --说明
        local cfg_name = "rise_arena_rank_award"
        local content = self.m_model.des_content
        --if self.m_model.m_match_type == 1 then
        --    cfg_name = "top_race_arena_reward"
        --    content = "tid#TianJiSai_des_6"
        --end
        local data = self.m_model.m_data
        data.last_settlement_time = self.m_model.m_data.end_ts --:getRemainingTime()
        self:openView("Arena.ArenaNormal.ArenaNormalReward", {open_type = 1, data = data, cfg_name = cfg_name, content = content,rise_id=self.m_model.m_rise_id})
    ...
    elseif msg=="refresh_ban_heros" then
        self.m_model:updateBanHeros(data.heros_ban)
    end
end

function M:checkDefendTeam()
    local team_num=self.m_model.team_num
    local set_flag = UserDataManager.hero_data:checkMultTeamNeedSetUp("rise_arena_def_mul", team_num)
    if set_flag then
        --self:openView("Arena.ArenaRace.ArenaTopDefendTeam", {rise_id=self.m_model.m_rise_id})
       self:openArenaHigherDefendTeam()
    end
    return set_flag
end
...
function M:openArenaHigherDefendTeam()
    self:openView("Arena.ArenaHigher.ArenaHigherDefendTeam",
            {is_liansaiZF=true,
              ban_heros=self.m_model.heros_ban,
              ban_num=self.m_model.ban_num,
              team_num=self.m_model.team_num,
              rise_id=self.m_model.m_data.rise_id,
              week_rule=self.m_model.m_week_rule,
...
function M:goodUser( userData )
    ...
    --点赞
    self.m_model:getNetData("rise_arena_do_like", params, function(data)
...
function M:arenaRecvWeekReward(data)
    ...
    local params = {award_id = data.id}
    self.m_model:getNetData("rise_arena_recv_week_award", params, netCallback)
end
...
function M:arenaRefreshTop()
    ...
    local params = {start = 1, stop = 3, match_type = self.m_match_type}
...
function M:arenaArenaIndex()
    local function receivetCallback(response)
        ...
        self:updateMsg("update_data",{ self_rank = self.m_model.m_data.rank,
                                       self_score = self.m_model.m_data.score,
                                       rise_id=self.m_model.m_data.rise_id,
                                       battle_mode=self.m_model.m_battle_mode,
                                       ban_num=self.m_model.ban_num},"Arena.ArenaRace.ArenaRaceChallenge")
    end
    local params = {}
    self.m_model:getNetData("rise_arena_index", params, receivetCallback)
end
