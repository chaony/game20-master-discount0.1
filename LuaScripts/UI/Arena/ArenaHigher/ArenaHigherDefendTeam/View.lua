---@class ArenaHigherDefendTeamView:OOPopBase
---@field m_model ArenaHigherDefendTeamModel
local M = class("ArenaHigherDefendTeamView",LikeOO.OOPopBase)

M.m_uiName = "Arena/ArenaHigher/ArenaHigherDefendTeam"
M.m_size_type = 2
...
function M:refreshUI()
	...
	if self.m_model.m_is_liansaiZF==nil then
		local edit_btn_node_trans=self:findGameObject("edit_btn_node").transform
		UIUtil.setLocalPosition(edit_btn_node_trans,-60,-231)
		local order_btn_trans=self:findGameObject("order_btn").transform
		UIUtil.setLocalPosition(order_btn_trans,0,-231)
	end
...
function M:updateBanView()
	self:setObjectVisible("ban_node",self.m_model.m_is_liansaiZF)
	if self.m_model.ban_num>0 then
		hero_node=self:findGameObject("hero_node_1").transform
		self:setBanHero(hero_node,self.m_model.ban_hero_ids[1])

		self:setObjectVisible("hero_node_2",self.m_model.ban_num==2)
		if self.m_model.ban_num==2 then
			hero_node=self:findGameObject("hero_node_2").transform
			self:setBanHero(hero_node,self.m_model.ban_hero_ids[2])
		end
	end
end

function M:setBanHero(node,hero_id)
	if hero_id then
		local hero_data=RewardUtil:getProcessRewardData({RewardUtil.REWARD_TYPE_KEYS.HEROS,hero_id, 0})
		GameUtil:updateItemElementByData(node,hero_data,false,false)
	else
		GameUtil:updateItemElementNoData(node)
	end
end
