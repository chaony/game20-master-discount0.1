local M = class("ArenaHigherChallengeControl",LikeOO.OOControlBase)
...
function M:onHandle(msg , data)
    ...
    elseif msg == "arena_cell_btn" then
        self:openView("Pops.PlayerInfo", {uid = data.user.uid, look_model = 2,show_battle_array = data.rank > 10})
        ...
        local function netCallback(response)
            --self:openView("Loading.BattleLoading", {callfunc = function(open_flag)
               -- if open_flag == "open_view" then
                    self:openView("Formation",{mode = GlobalConfig.BATTLE_MODE.HIGH_ARENA, 
                                               defend_uid = data.user.uid, def_data = response, formation_index = 1, auto_battle_flag = auto_battle_flag,show_def_data = data.rank > 10 and 0 or 1})
               -- end
           -- end })
        end
            ...
            local function netCallback(response)
                if response.update then -- 需要重新匹配
                    ...
                    if response.need_battle == 1 then -- 需要战斗
                        ...
                        self:openView("Settlement", { result = 1, mode = GlobalConfig.BATTLE_MODE.HIGH_ARENA, battle_data = {}, quick_pass = true, data = response, full_mask_flag = true})
                    end
                end
            end
            local params = {defend_uid = data.user.uid}
            self.m_model:getNetData("high_arena_quick_pass_arena", params, netCallback)
        else
            self:openView("Arena.ArenaNormal.ArenaNormalTicketBuy", {buy_mode = 1, buy_times = self.m_model.m_data.buy_times})
        end
    end
