local M = class("ArenaHigherRewardView",LikeOO.OOPopBase)
...
function M:onEnter()
	...
	self:setTextByLanKey("score_label_text", "new_str_0886", tostring(self.m_model.m_data.score or 0))
	if self.m_model.m_data.rank <= 0 then
		...
		CommonUIUtil:setSegmentInfo(segment_node, cfg, false)
	end
	self.m_toggle_btns = {}
...
function M:switchTabNode(index)
	...
	if index == 1 then
		self:updateDailyQuestsLoopScroll()
	elseif index == 2 then
...
function M:updateDwScrollViewCell(index, cell_object, cell_data)
	...
	local cfg = ConfigManager:getHighArenaCfgByRankMark(rank)
...
function M:refreshRedPoint()
	local record_flag1 = RedPointUtil:hasRedPointById(2402)
	local record_flag2 = RedPointUtil:hasRedPointById(2403)
	self:setObjectVisible("togglebtn_red_point_1", record_flag1)
	self:setObjectVisible("togglebtn_red_point_2", record_flag2)
end
