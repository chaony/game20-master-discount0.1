---@class RTAAdvanceView:OOPopBase
---@field m_model RTAAdvanceModel
local M=class("RTAAdvanceView",LikeOO.OOPopBase)


M.m_size_type = 2
M.m_iphoneXAdapter = true
M.m_uiName = "Arena/ArenaRTA/RTAAdvancePop"

function M:onEnter()
    --local lan_index=self.m_model.match_type+59
    --local lan_key="arena_str_00"..lan_index
    self:setTextByLanKey("main_node_tips_text","arena_rta_str_0030")
    ...
    self.limit_icon_trans=self:findGameObject("new_limit_icon").transform

    --人，黄，玄，地，天12345
    self.race_icon={
        [1]={name_icon="ljsz_zfls_name1",badge_icon="rta_dan_badge_qingtong"},
        [2]={name_icon="ljsz_zfls_name2",badge_icon="rta_dan_badge_baiyin"},
        [3]={name_icon="ljsz_zfls_name3",badge_icon="rta_dan_badge_gold"},
        [4]={name_icon="ljsz_zfls_name4",badge_icon="rta_dan_badge_zuanshi"},
        [5]={name_icon="ljsz_zfls_name5",badge_icon="rta_dan_badge_dashi"},
    }
    ...
    elseif self.m_model.promote_flag==4 then--失败晋级
        ...
        self:setTextByLanKey("failure_node_tips_text","arena_rta_str_0029")
    ...
    elseif self.m_model.promote_flag==1 then
        self:promote(self.m_model.match_type+1)
        self.m_control:setOnceTimer(3, function()
            self.m_control:updateMsg(99999)
        end)
    end
...
function M:initMainNode(next_match_type)
    ...
    --self:setObjectVisible("banXuan",false)
    --self:setObjectVisible("multiTeam_battle",false)
...
function M:initPromoteEffectItem(type,item_obj_name)
    ...
    if is_contain then
        ...
        if type==self.PROMOTION_EFFECT_TYPE.liansaiguize then
            ...
            local icon_name=rule_cfg[id].rule_icon
        ...
        elseif type==self.PROMOTION_EFFECT_TYPE.daoju then
            ...
            GameUtil:updateItemElement(trans,data,false,true)
...
function M:initDuanWei(match_type)
    ...
    self:setObjectVisible("UI_Arena_Xduawei0"..match_type,true)
