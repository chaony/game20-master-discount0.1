local M = class("ArenaRaceModel", LikeOO.OODataBase)

function M:onCreate()
	-- self.m_transfer = "up_to_down"
	M.super.onCreate(self)
	--获取种族竞技场接口
	self:getData("race_arena_race_arena_index")
end

function M:onEnter()
	self.races = self.m_data.races
	self.m_match_type = self.m_data.match_type and self.m_data.match_type or 0  -- 0 地级赛 1 天级赛
	self:sortRanksTops()
end
...
function M:initArenaRewardWeekData()
	...
	local arena_reward_week = ConfigManager:getCfgByName("race_arena_reward_week")
