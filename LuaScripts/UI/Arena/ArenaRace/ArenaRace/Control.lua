---@class ArenaRaceControl:OOControlBase
local M = class("ArenaRaceControl",LikeOO.OOControlBase)
...
function M:onHandle(msg , data)
    ...
    elseif msg == "def_btn" then
        if self.m_model.m_match_type == 1 then
            self:openView("Arena.ArenaRace.ArenaTopDefendTeam", { mode = GlobalConfig.BATTLE_MODE.TOP_RACE_ARENA_DEFENSE, races = self.m_model.races })
        else
            self:openView("Formation",{mode = GlobalConfig.BATTLE_MODE.RACE_ARENA_DEFENSE, races = self.m_model.races})
        end
    ...
    elseif msg == "challenge_btn" then
        if self.m_model.m_match_type == 1 and self:checkDefendTeam() then
        else
    ...
    elseif msg == "explain_btn" then --说明
        local cfg_name = "race_arena_reward"
        local content = "tid#arena_explain8"
        if self.m_model.m_match_type == 1 then
        ...
        self:openView("Arena.ArenaNormal.ArenaNormalReward", {open_type = 1, data = data, cfg_name = cfg_name, content = content})
...
function M:goodUser( userData )
    ...
    --点赞
    self.m_model:getNetData("race_arena_like", params, function(data)
...
function M:arenaRecvWeekReward(data)
    ...
    self.m_model:getNetData("race_arena_recv_week_reward", params, netCallback)
end
...
function M:arenaArenaIndex()
    ...
    local params = {}
    self.m_model:getNetData("race_arena_race_arena_index", params, receivetCallback)
end
