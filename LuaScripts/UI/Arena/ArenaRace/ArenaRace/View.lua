...
M.m_uiName = "Arena/ArenaRace/ArenaRacePop"
M.m_size_type = 1
...
function M:onEnter()
	...
	self:setObjectVisible("top_shili", self.m_model.m_match_type == 1)
	self:setObjectVisible("team_name_text", self.m_model.m_match_type ~= 1)
	for i = 1, 2 do
		self:setObjectVisible("team_shili_text" .. i, self.m_model.m_match_type == 1)
		self:setTextByLanKey("team_shili_text" .. i, "arena_str_0026", i)
	end
	UserDataManager:removeRedDotByKey("race_arena")
	local bg_img = self:findImage("Panel")
	local bg_img_name = self.m_model.m_match_type == 1 and "a_wxljtjs_bg" or "a_wxlj_bg"
	GameUtil:updateResourcesImg( bg_img, "Texture/arena/" .. bg_img_name)
	local self_node_bg_img = self:findImage("self_node_bg_img")
	local self_node_bg_img_name = self.m_model.m_match_type == 1 and "a_ljszh_renwu2" or "a_ljszh_renwu"
	GameUtil:updateResourcesImg( self_node_bg_img, "Texture/arena/" .. self_node_bg_img_name)
end
...
function M:initUi()
	...
	for i = 1,3 do
		LuaBehaviourUtil.setObjectVisible(self.m_rank_behaviour[i], "good_red_point_"..i,  RedPointUtil:hasRedPointById(11501) == true)
	end
end
...
function M:updateRankBg()
	...
	for i = 1, 3 do
		local bg_name = img_name[self.m_model.m_match_type + 1][i]
...
function M:updateArenaRewardWeek()
	...
	for i=1,box_num do
		...
		local cfg = data.cfg
		local task_box = GameUtil:createPrefab("Arena/ArenaRace/ArenaRewardWeekBox", box_trans)
