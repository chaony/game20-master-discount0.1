---@class ArenaRaceChallengeControl:OOControlBase
---@field m_model ArenaRaceChallengeModel
local M = class("ArenaRaceChallengeControl",LikeOO.OOControlBase)
    ...
    if data then
        ...
        if free_time > 0 or item_data.num > 0 then
            local function netCallback(response)
                --Logger.logError(response," 开始挑战种族竞技场 ")
                if self.m_model.m_match_type == 1 then
