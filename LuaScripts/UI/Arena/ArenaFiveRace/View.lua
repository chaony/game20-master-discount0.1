local M = class("ArenaRaceView",LikeOO.OOPopBase)

M.m_uiName = "Arena/ArenaFiveRace/ArenaFiveRace"
M.m_size_type = 1
M.m_iphoneXAdapter = true

function M:onEnter()
	self.m_attr_node = GameUtil:commonAttrNode(self.m_control, {mode = 1, item_id = 1033})
	self.m_attr_node:setTitle(Language:getTextByKey("tid#arena_explain7"))
	self:setTextByLanKey("list_title_text", "new_str_1058")
	...
	self:setTextByLanKey("day_reward_text", "new_str_1063", cur_num, max_num)
	...
	self:initUi()
	self:refreshUI()
	self:setObjectVisible("shili", self.m_model.m_match_type ~= 1)
	self:setObjectVisible("team_name_text", self.m_model.m_match_type ~= 1)
end

function M:initUi()
	self:setTextByLanKey("close_title_text", "arena_str_0030")
	--排名1 2 3 的格子 
	...
	self:setObjectVisible("frist_rank", true)
	self:setObjectVisible("second_rank", true)
	self:setObjectVisible("third_rank", true)
	self.m_rank_behaviour = {
		[1] = self.m_frist_rank,
		[2] = self.m_second_rank,
		[3] = self.m_third_rank,
	}
	self.m_good1 = self.m_frist_rank:FindGameObject("good1")
	self.m_good2 = self.m_second_rank:FindGameObject("good2")
	self.m_good3 = self.m_third_rank:FindGameObject("good3")
	self.m_good_list = {
		[1] = self.m_good1,
		[2] = self.m_good2,
		[3] = self.m_good3,
	...
	for i = 1,3 do
		LuaBehaviourUtil.setObjectVisible(self.m_rank_behaviour[i], "good_red_point_"..i,  RedPointUtil:hasRedPointById(23401) == true)
	end
end
	...
	for i = 1, 3 do
		...
		if rank_data ~= nil then
			if rank_data.user.is_robot == true then
				...
				LuaBehaviourUtil.setObjectVisible(self.m_rank_behaviour[i],"good"..i, false)
...
function M:updateRaceData()
	...
	for i = 1, loop_num do
		self:setTextByLanKey("team_text" .. i , team_name[i])
		local races_node = self:findGameObject("races_node" .. i)
		local race_luabehaviour =  UIUtil.findLuaBehaviour(races_node.transform)
		--local race_img1 = race_luabehaviour:FindImage("race_img1")
		--local race_img2 = race_luabehaviour:FindImage("race_img2")
		local race_index = i
		
		if top_data[race_index] then
			local races = top_data[race_index]
			for j = 1, 2 do
				if races[j] then
					local race_img_info = GlobalConfig.TYPE_HERO_RACE[races[j]];
					LuaBehaviourUtil.setImg(race_luabehaviour, "race_img" .. j, race_img_info.arena_icon, "arena_ui")
					LuaBehaviourUtil.setObjectVisible(race_luabehaviour, "race_img" .. j, true)
				else
					LuaBehaviourUtil.setObjectVisible(race_luabehaviour, "race_img" .. j, false)
				end
...
function M:updateRank( cur_LuaBehaviour, rank_data )
	if cur_LuaBehaviour ~= nil then
		...
		GameUtil:setUserAvatar(HeadNode, rank_data.user, nil, nil, {show_flag = true, scale = 0.65})
...
function M:updateSelfData()
	...
	local score_text = luaBehaviour:FindText("main_score_text")
	...
	GameUtil:setUserAvatar(HeadNode, user_data, nil, nil, {show_flag = true, scale = 0.75})
	local title_id = user_data.title
	
	if title_id and title_id ~= 0 then
		HeadNode.transform.anchoredPosition = Vector3.New(0, 200, 0)
	else
		HeadNode.transform.anchoredPosition = Vector3.New(0, 174, 0)
	end
...
function M:refreshBoxStatus()
	...
	self:setTextByLanKey("task_nums_text", cur_num .. "/" .. max_num)
	for i = 1, max_index do
		...
		if data.status == -1 then
			LuaBehaviourUtil.setImg(luaBehaviour, "box_img", "a_xwyj_phb_cheston", "mystic_ui")
		else
			LuaBehaviourUtil.setImg(luaBehaviour, "box_img", "a_xwyj_phb_chestoff", "mystic_ui")
		end
	...
	self.m_new_box_reward_slider.value = cur_num / max_num
end
