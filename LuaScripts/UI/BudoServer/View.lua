local M = class("BudoServerView", LikeOO.OOPopBase)

M.m_uiName = "BudoServer/BudoServer"
M.m_size_type = 1
M.enemy_hero_list = {}
...
function M:onEnter()
    ...
    self:setTextByLanKey("close_title_text", "budoServer_text_0007")
    self.enemy_hero_list = {}
  
    self:setTextByLanKey("rank_btn_text", "budoServer_text_0003")
    self:setTextByLanKey("gift_btn_text", "budoServer_text_0002")
    self:setTextByLanKey("hero_btn_text", "budoServer_text_0001")
    self:setTextByLanKey("cur_floor_count", "tid#towertip_01")
    self:setTextByLanKey("reward_des_text", "tid#TianJiMiJingDes_002")
...
function M:refreshRedPoint()
    local is_red = RedPointUtil:hasRedPointById(24501)
    local is_red2 = RedPointUtil:hasRedPointById(24502)
    self:setObjectVisible("gift_red_point", is_red or is_red2)
end

function M:updateTime()
    local temp = self.m_model:getEndTs()
    if self.m_model:getEndTs() > 0 then
        local tim = GameUtil:formatTimeBySecond(self.m_model:getEndTs())
        self:setTextByLanKey("time_des_text", Language:getTextByKey("budoServer_text_0019", tim))
    else
        self:setTextByLanKey("time_des_text", Language:getTextByKey("budoServer_text_0018"))
    end
end
...
function M:refreshUI()
    ...
    if self.m_model:checkCanQuick() == true then
        self:setTextByLanKey("challenge_btn_text", "快速挑战")
    else
    ...
    if self.m_model.cur_floor >= self.m_model.max_floor then
        ...
        local dianfeng_text = self:setObjectVisible("dianfeng_text", true)
        dianfeng_text.transform.localPosition = Vector3(-146.98, 65, 0)
...
function M:moveToFloor(floor)
    if self.m_scroll_view then
        ...
        for k, v in pairs(self.m_model.enemy_list) do
            if v == floor then
                self.m_scroll_view:moveToCellIndex(k - 1 > 0 and k-1 or 1)
            end
        end
    end
...
function M:updateRightCount(select_floor)
    ...
    if battle_data then
        ...
        Logger.logError(next_data,"  没找到数据 ")
    end
    ...
    if self.m_scroll_view == nil then
        ...
        local params = {
            ...
            update_cell = function(index, cell_obj, cell_data)
                ...
                if LuaBehaviour then
                    LuaBehaviourUtil.setObjectVisible(LuaBehaviour, "ta_bg_1", true)
                    LuaBehaviourUtil.setObjectVisible(LuaBehaviour, "blue_light", false)
                    LuaBehaviourUtil.setObjectVisible(LuaBehaviour, "cur_img", false)
                    LuaBehaviourUtil.setObjectVisible(LuaBehaviour, "cur_bg_img", false)
                    LuaBehaviourUtil.setObjectVisible(LuaBehaviour, "cur_bg_img2", false)
                   
                    local tower_drop = LuaBehaviour:FindGameObject("tower_cell_reward")
                    UIUtil.destroyAllChild(tower_drop.transform)
                    local user = self.m_model:getTowerDataByLayer(cell_data)
                    if user then
                        self.m_model:setTempUser(user)
                        LuaBehaviourUtil.setObjectVisible(LuaBehaviour, "first_img", true)
                        local head_node = LuaBehaviour:FindGameObject("head_node")
                        GameUtil:setUserAvatar(head_node, user.user, false,nil)
                        LuaBehaviourUtil.setText(LuaBehaviour, "player_name_text", user.user.name)
                    else
                        LuaBehaviourUtil.setObjectVisible(LuaBehaviour, "first_img", false)
                    end
                    ...
                    if cell_data > self.m_model.cur_floor then
                        ...
                        if cell_data == self.m_model.cur_floor + 1 then
                            ...
                            LuaBehaviourUtil.setObjectVisible(LuaBehaviour, "cur_img", true)
                            LuaBehaviourUtil.setObjectVisible(LuaBehaviour, "cur_bg_img", true)
                            LuaBehaviourUtil.setObjectVisible(LuaBehaviour, "cur_bg_img2", true)
                        else
