...
function M:init()
    self.m_is_vaild = GameVersionConfig.GUIDE_OPEN ~= false
    -- 解析配置信息
    self:analyseCfg()
    self:getStartTeamId()
    -- 重置新手引导
...
--设置初始引导id
function M:getStartTeamId()
    if GameVersionConfig.Is_BIGGAMEAPP then
        self.m_start_team_id =  ConfigManager:getCommonValueById(800, 1001)
    else
        self.m_start_team_id =  ConfigManager:getCommonValueById(438, 0)
    end
end
...
function M:analyseCfg()
    ...
    for id, cfg in pairs(__guide_team_cfg) do
        ...
        if GameVersionConfig.Is_BIGGAMEAPP  then
            if cfg.client_type == 2 or cfg.client_type == 3 then
                table.insert(team_ids,params)
            end
        else
            if cfg.client_type == 1 or cfg.client_type == 3 then
                table.insert(team_ids,params)
            end
        end
    end
    local sortFunc = function (data1, data2)
        if data1.sort < 0 or data2.sort < 0 then
...
function M:resetCurGuide( stype )
    if self.m_cur_guide then
        ...
        if teamCfg == nil or (teamCfg.type ~= 2 and teamCfg.type ~= 4 and teamCfg.type ~= 5) then
            return
        end
    end
    local __guide_cfg = ConfigManager:getCfgByName("guide")
    ...
    for i = index, #self.m_team_ids do
        ...
        local curFlag = false
        if teamCfg.type == 1 or teamCfg.type == 3 then -- type=2 不进行主动新手引导
            curFlag = true
        end
        if stype == true then -- stype 在特定条件下触发 type=4 得新手引导
            curFlag = true
        end
        if teamCfg.open_level <= level  and curFlag == true then  -- 已经开启
            -- 检查是否已经引导完成
            local teamId = idInfo.id
            local sortIds = self.m_sort_ids[idInfo.sort]
            if sortIds and #sortIds > 0 then
                local server = guideServer[tostring(idInfo.id)]
    ...
    if curTeamId then
        ...
        for i = index, #self.m_team_ids do
            ...
            local teamCfg = __guide_team_cfg[idInfo.id]
            local curFlag = false
            if teamCfg.type == 6 then -- type=4 不进行主动新手引导
                curFlag = true
            end
            if stype == true then -- stype 在特定条件下触发 type=4 得新手引导
                curFlag = true
    ...
    if self.m_is_vaild and realIndex == 0 and #self.m_team_ids > 0 and curTeamId == self.m_start_team_id then
...
function M:getCurTeamId(teamId, sortIds)
    ...
    if teamCfg.type == 3 or teamCfg.type == 6 then -- 带有触发条件的引导，3强引导，6若引导
        ...
        if trigger_type == 1 then -- 通过触发
            local open_condition = trigger[3]
            local opened = true
            if open_condition and open_condition > 0 then
                opened = BtnOpenUtil:isBtnOpen(open_condition)
            end
            if opened then
                local stage = UserDataManager:getCurStage()
                -- Logger.log(stage,"stage ======")
                if teamCfg.type == 6 then
...
--- 设置任意组引导
function M:setAnyTeamGuide(teamId, guide_index)
    teamId = tonumber(teamId)
    local level = UserDataManager.user_data:getUserStatusDataByKey("level") or 1
    local guideServer = UserDataManager:getGuide()
    local realIndex = guide_index or 0
    local __guide_cfg = ConfigManager:getCfgByName("guide")
    local __guide_team_cfg = ConfigManager:getCfgByName("guide_team")

    local teamCfg = __guide_team_cfg[teamId]
    local isHave = false
    if teamCfg and teamCfg.open_level <= level then  -- 已经开启
        local sortIds = self.m_sort_ids[teamCfg.sort]
        if sortIds and #sortIds > 0 then
            local server = guideServer[tostring(teamId)]
            if server then  -- 完全没有记录 开始进行引导
                ...
                for index,id in ipairs(sortIds) do
                    ...
                    if cfg.aim == 1 and id > server then
                        ...
                        break
                    elseif id <= server then
                        realIndex = math.max(index, realIndex)
                        if cfg.aim == 1 then--跳过
                            break
                        end
                    end
...
-- 向服务器发送已完成引导数据
function M:sendGuideData(skip_value)
    ...
    if guide_id > server_id then
        local params = {sort = cfg.sort, guide_id = guide_id, skip = skip_value or 0}
        ...
        Logger.log(params, "send user_guide -->")
...
function M:skipAllGuide()
    if self.m_cur_guide then
        if not self.m_cur_guide.free then
            ...
            local key = NetUrl.getUrlForKey("user_guide")
            NetWork:httpRequest(responseMethod, key, GlobalConfig.POST, {skip = 1}, key, 3, true)
        end
    end
...
function M:getLastNewGuideSortAndID()
    local cfg_guide_team = ConfigManager:getCfgByName("guide_team")
    local cfg_guide = ConfigManager:getCfgByName("guide")
    local info_guide_team = cfg_guide_team[table.nums(cfg_guide_team)]
