...
function TimeUtil.fmtTime(time, fmtType)
    ...
    elseif fmtType == 5 then
        return os.date("%Y".."-%m".."-%d".. " %X", math.ceil(time))  --2023-8-31 05:24:12
	end	
end

-- 时间戳获取当天0点的时间戳
function TimeUtil.getIntTimestamp(time, timezone)
    timezone = timezone and timezone or UserDataManager:getTimeZone()/3600
    return time - (time + timezone*3600)%86400
end
