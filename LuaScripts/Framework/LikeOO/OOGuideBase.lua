...
function M:checkGuide(...)
    ...
    if not UserDataManager.guide_data:isGuiding() then
        ...
        if not UserDataManager.guide_data:curGuideIsForce() then -- 不是硬引导检测是否继续引导
...
function M:realCheck(...)
    local info = self:hasGuide()
     --Logger.log(self.__cname,"+++++++++++++++++++++++++++realCheck guide key -->")
     --Logger.log(info,"+++++++++++++++++++++++++++++++realCheck guide info -->")
    if info then
        self:markGuide()
        ...
        if self:getExcuteFunc(info) then
            if self.m_control:hasChild("Guide.GuideDialog") and self.m_before_guide_info then
                if info.action == self.m_before_guide_info.action and info.key == self.m_before_guide_info.key then
                    Logger.logWarning(info, "Guide.GuideDialog is same guide ")
                    return
                end
            end
            -- 添加容错。
...
--- 直接返回主界面
function M:excuteGuideFunc99998(info)
    static_rootControl:closeAllViewPop()
    self:doNextGuide()
    static_rootControl:updateMsg("closeNode_btn")
    static_rootControl:updateMsg("common_refresh")
end
...
function M:excuteGuideFunc99999(info)
    ...
    if btn then 
        self:guideTargetNode(btn.transform, 1, 1)
    end
end
...
function M:doNextGuide()
    ...
    if last_info and last_info.reward then
        ...
        if #reward > 0 and reward[1][1] == RewardUtil.REWARD_TYPE_KEYS.HEROS then
            ...
            self.m_control:openView("HeroInfo.HeroNewPop", {hero_id = reward[1][2], is_new = true})
        end
    end
    local current_info = UserDataManager.guide_data:getCurGuideInfo()
    if not current_info then        -- 当前系列新手引导结束后，取消锁定touch
        local control = static_rootControl:getRootControl()
        control.m_view:unlockTouch("TAG:next dialog")
        G_GUIDE_FORCE_NET_COMPLETE = false
    end
...
-- 通过交互节点创建交互层
function M:guideTargetNode(targetTrans, maskType, eventType, speed, finger, finger_offset, forceSize, target_pos, close_skip, clickCallFunc, other_params)
...
-- 显示小游戏交互层
function M:showGuideGameDialog(control, params)
    static_rootControl.m_view:lockTouch("TAG:will dragDialog")
    static_rootControl:openView("Guide.GuideGameDialog", params);
    static_rootControl.m_view:unlockTouch("TAG:will dragDialog")
end

-- 通过交互节点创建交互层 小游戏
function M:guideTargetGameNode(targetTrans, maskType, eventType, speed, finger, finger_offset, forceSize, target_pos, close_skip, clickCallFunc, other_params)
    local info = self.m_excute_info
    if targetTrans then
        local params = {
            target_trans = targetTrans,
        ...
        self:showGuideGameDialog(self.m_control, params);
    else
        self:doNextGuide()
    end
end
...
function M:checkListenerTrigger(msg, data)
    if self.m_listener then
        if self.m_listener.key == msg or self.m_listener.key2 == msg then
