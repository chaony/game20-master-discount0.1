---@class OOViewBase
---@field m_control OOControlBase
---@field m_model OOViewBase
---@field m_luaBehaviour CS.wt.framework.LuaBehaviour
...
function M:create()
	...
	if self.m_uiName then
		...
		self.m_canvas.sortingOrder = self.m_sortOrder % 32768
		...
		local aspect_ratio = self.m_view_width/self.m_view_height
		...
		-- end
		self.screen_shot_rawimg = self:findGameObject("screen_shot_rawimg")
		...
		self.m_iphonex_offset_x = 0
		-- 1280 - 1380
		if self.m_iphoneXAdapter then
			if is_iphonex then
				self.m_iphonex_offset_x = GlobalConfig.UI_LEFT_OFFSET_X
			else
				local diff_aspect_ratio = aspect_ratio - 1380/GlobalConfig.UI_DESIGN_HEIGHT
				if diff_aspect_ratio > 0 then
					self.m_iphonex_offset_x = GlobalConfig.UI_LEFT_OFFSET_X -- diff_aspect_ratio*360
				end
			end
		end
		...
		canvas.sortingOrder = (self.m_sortOrder + __VIEW_SORT_ORDER_STEP - 1) % 32768
...
function M:resetVisibleView()
	if self.m_visibleRefCount < 0 then
		self.m_visibleRefCount = 0
		self.m_rootView:SetActive(true)
	end
end
...
function M:openTransition( callBackFunc )
	...
	if not IsNull(self.screen_shot_rawimg) and self.screen_shot_rawimg.activeInHierarchy then
		screenShot = self.screen_shot_rawimg:GetComponent("ScreenShot")
	end
	if not IsNull(screenShot) then
		local function shotback()
			self:runOpenAnim(self.content_node, callBackFunc)
		end
		screenShot:ShotScreen(shotback)
	else
		self:runOpenAnim(self.content_node, callBackFunc)
	end
end
