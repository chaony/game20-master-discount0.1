---@class OODataBase
---@field m_name string 名字
---@field m_alias string 别名
---@field m_data  table
...
function M:getData( key , params, loadingFlag, requestMethod, ext, pop_failed_tips_count)	
	...
	if url~=nil then
		local function responseMethod(data, tag, status_code, pop_failed_tips_count)
			if(data == nil )then
				if ext and ext.forceBack then   -- 强制返回
					self:getData(key , params, loadingFlag, requestMethod, ext, pop_failed_tips_count)
				else
		...
		NetWork:httpRequest( responseMethod , url , requestMethod , params , key , loadingFlag or 1 , true, nil, nil, nil, pop_failed_tips_count)
	else
		self:callBack(nil)
	end
end
...
-- 获取网络数据，有回调
function M:getNetData( key , params , callfunc , loadingFlag, returnFlag, requestMethod, ext, pop_failed_tips_count, retry_count)
	local url = NetUrl.getUrlForKey(key)
	local function responseMethod(data, tag, status_code, pop_failed_tips_count)
		if(data ~= nil)then
        	...
        	if callfunc and self.m_destroy_flag == false then
       		...
       		if ext and ext.forceBack then   -- 强制返回
       			self:getNetData(key , params , callfunc , loadingFlag, returnFlag, requestMethod, ext, pop_failed_tips_count)
       		else
	       		if callfunc and self.m_destroy_flag == false then
	...
	NetWork:httpRequest( responseMethod , newkey , requestMethod , params , key, loadingFlag or 1, returnFlag, nil, nil, retry_count, pop_failed_tips_count)
end
...
--加载配置，销毁ui时会自动调用配置的卸载
function M:loadCfg(key)
	local cfg = ConfigManager:getCfgByName(key)
	if self.m_cfg_keys[key] == nil then
		ConfigManager:retainCfgByName(key)
		self.m_cfg_keys[key] = 1
	end
	return cfg
end

function M:unloadAllCfg()
	for k,v in pairs(self.m_cfg_keys) do
		ConfigManager:releaseCfgByName(k)
	end
end
