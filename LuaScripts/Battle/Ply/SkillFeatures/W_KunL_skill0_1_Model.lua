...
---@class W_KunL_skill0_1_Model : SkillFeatures_Model @
---@field super SkillFeatures_Model @SkillFeatures_Model
local M = class("W_KunL_skill0_1_Model", SkillFeatures_Model)
...
function M:skillDispatch(data)
    if data.eventName == "skill0_use" then
        if self.target ~= nil then
            self.target.bufMgr:addBufById(self.critBuff, self.player, self.skill)
        end
    end
end
...
function M:killerAfterAttack(data)
    ...
    if self.player:equal(killer) and skill == nil or skill.anim_name == "skill0" then
        victim.bufMgr:addBufById(self.critBuff, self.player, self.skill)
    end
end
