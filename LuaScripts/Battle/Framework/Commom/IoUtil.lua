...
function io.exists(path)
    if GameVersionConfig.IS_SERVER then
        ...
        Logger.log( GameVersionConfig.LUA_ROOT_PATH.. path )
        return false
    end
...
function io.fileFullPath(path)
    ...
    if CS.wt.framework.LuaFileHelper.Inst:FileExist(path, CS.wt.framework.LuaFileHelper.DirectoryType.ReadAndWritePath) then -- 可读写
        ...
        file_type = 2;
    elseif CS.wt.framework.LuaFileHelper.Inst:FileExist(path, CS.wt.framework.LuaFileHelper.DirectoryType.ReadOnlyPath) then -- 只读
...
--写入战斗log
function io.writebattlelog(path, content, mode)
    local log_path = GameVersionConfig.BATTLE_LOG_PATH..path
    if GameVersionConfig.IS_SERVER then
        --Logger.logError(" 游戏结束写入log ~~~~~~~~~~~~~ "..path )
        mode = mode or "w+b"
        local file = io.open(log_path, mode)
        if file then
            if file:write(content) == nil then return false end
            io.close(file)
            Logger.log("服务器文件 "..log_path.."导出成功")
            return true
        else
            return false
        end
    else
        local fs = CS.wt.framework.LuaFileHelper.Inst:OpenFile(log_path, 2)
        CS.wt.framework.LuaFileHelper.Inst:WriteText(fs,content)
        CS.wt.framework.LuaFileHelper.Inst:CloseStream(fs)
        Logger.log("客户端文件 "..log_path.."导出成功")
    end
end
