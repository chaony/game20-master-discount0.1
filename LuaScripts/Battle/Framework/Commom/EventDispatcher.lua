...
---@field eventList table
---@field allEventMsgList table
...
EventDispatcher.eventDispatchNum = {}
--[[
    说明:  监听时间事件
          eventname  事件名称
          interval   事件间隔
...
--- 设置某个事件的当前时间
function EventDispatcher:setTimeDuration(eventName, duration, eclips)
    local time = self.timeList[eventName]
    if time then
        time.duration = duration
        time.eclips = eclips or time.eclips
    end
end
...
--清理发送次数
function EventDispatcher:clearDispatchNum()
    EventDispatcher.eventDispatchNum = {}
end


--发送次数记录
function EventDispatcher:dipatchEventNum(eventName)
    local eventNameItem = EventDispatcher.eventDispatchNum[eventName];
    if eventNameItem == nil then
        eventNameItem = {}
        eventNameItem.count = 1;
        EventDispatcher.eventDispatchNum[eventName] = eventNameItem;
    else
        eventNameItem.count = eventNameItem.count + 1;
    end
end
...
function EventDispatcher:resetAllList()
    self.msgList = {}
    self.timeList = {}
    self.eventList = {}
    self.tickDiction = {}
    self.allTimeMsgList = {}
    self.allEventMsgList = {}
    self.eventDispatchNum = {}
end
