--异界迷宫
---@class MiGongScene_Model : Scene_Model @
---@field super Scene_Model @Scene_Model
local M = class("MiGongScene_Model",Battle.Scene_Model)
...
function M:loadFinish()
    ...
    --箭头每0.5秒矫正一次方向
    self:resetSceneData()
    self:sendEvent("load_finish", nil, "MazeStage");
end
...
function M:resetSceneData( data )
    ...
    TimeTools:killModel()
    --发送到迷宫清理格子
    self:sendEvent("maze_clear_grid", nil, "MazeStage")
    --
    if data ~= nil then
    ...
    local maze_prmap = ConfigManager:getCfgByName("maze_prmap")
    ...
    self.localConfig = require("Battle.Data.FuBen."..tostring(self.map_id));
    --格子位置信息
    self.MapPos = require("Battle.Data.FuBen.MapPos")
    --初始化AStar
...
function M:UnityUpdate(dt)
    ...
    if static_rootControl:can3DTouchByViewName("MazeStage") then
        ...
        if self.nextGrid == nil then
            ...
            if self.path.Count > 0 then
                ...
                if self.path.Count >= 5 then
                    ...
                    --迷宫小地图用的
                    --self:sendEvent("maze_move_grid", center_cell_data, "MazeStage");
...
function M:createMigongPlayer()
    ...
    if self.player == nil then
        ...
        self:sendEvent("set_grid_position", cell_data, "MazeStage")
        --起始格子
            ...
            if is_guide == false and v.server_data then
                ...
                elseif v.server_data.type == 13 then -- 出口
                    ...
                    if self.pass_status == 0 then
                        ...
                        static_rootControl:updateMsg("upAllCells",self,"MazeStage")
                        self.pass_status = 1
                    end
                end
            end
...
function M:handlerPassedGrid()
    if self.cur_floor_data.fog == 0 then
        --根据服务器的逐步打开
...
function M:findPaths( target_w, target_h )
    ...
    if self.path.Count <= 0 then
        static_rootControl:updateMsg("show_no_move_messag",nil,"MazeStage")
        self:resetClickRoom();
        self.isTranfering = false;
    end
end
...
function M:sendToServer()
    ...
    if self.movePassPath.Count > 0 then
        ...
        self:sendEvent("maze_goto",path_data,"MazeStage")
...
function M:selectCore( obj, data, id )
    ...
    self:sendEvent("maze_goto_guide",nil,"MazeStage")
...
function M:clickObjectMode2(w, h, center, around, obj, m_type)
    ...
    if self.select_map_data ~= nil and self.select_map_data.item ~= nil and self.select_map_data.item:isCanClick() then
        ...
        static_rootControl:updateMsg("cell_click",data,"MazeStage")
    end
end
...
function M:sendToServerAll()
    ...
    if self.canClickType[cell_type] ~= nil then
        self:clickSure(cell_type, function()
            self:sendToServer();
            self:sendEvent("cell_click",data,"MazeStage")
        end)
    end
end
...
function M:upToObj( objData, isDrect, finish )
    if objData.obj ~= nil and IsNull(objData.obj) == false then
        ...
        self:sendEvent("maze_show_grid", objData, "MazeStage")
    end
end
...
function M:setObjData( m_obj, w_index, h_index )
    ...
    if grid_data ~= nil and grid_data.group < 99 then
        ...
        --迷宫Item, 在平台上的物件
        local m_item = require("Battle.Sce.Tools.MiGongItem").new();
