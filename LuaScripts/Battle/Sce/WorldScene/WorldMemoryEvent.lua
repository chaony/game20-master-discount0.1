...
local M = class("WorldMemoryEvent")
        ...
        if event_before > 0 then
            if self.event_type == GlobalConfig.WORLD_MAP_EVENT.REGIONAL_EVENT and self.eventData.isArticle ~= true then
                if self.eventData.option_switch == 1 then
                    local article_option = ConfigManager:getCfgByName("new_regional_article_option")
                    local choise = {}
                    local choise_id = {}
...
function M:talk(talk_id, choise, choise_id, can_choise, no_choise_tips)
    ...
    static_rootControl:openView("Guide.GuideDrama", {dialog_id = data.talk_id, callback = callback, choise = data.choise, choise_id = data.choise_id, can_choise = can_choise, no_choise_tips = no_choise_tips, dialogue_type = data.dialogue_type, waitForChoise = data.waitForChoise, worldMemory = self.worldMemory}, nil , true)
end
