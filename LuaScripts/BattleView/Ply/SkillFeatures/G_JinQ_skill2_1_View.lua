---@class G_JinQ_skill2_1_View : SkillFeatures_View @
---@field super SkillFeatures_View @SkillFeatures_View
local M = class("G_JinQ_skill2_1_View", SkillFeatures_View)

function M:init(player, skill, model)
    M.super.init(self, player, skill, model)
    self.show = false
    self:addEventListener_Local(Battle.SkillEventType.MV_G_JinQ_skill2_1_Model_ShowEffect,{self, self.MV_G_JinQ_skill2_1_Model_ShowEffect})
    self:addEventListener_Local(Battle.SkillEventType.MV_G_JinQ_skill2_1_Model_CreateFootEffect,{self, self.MV_G_JinQ_skill2_1_Model_CreateFootEffect})
end

--金钱显示特效
function M:MV_G_JinQ_skill2_1_Model_ShowEffect(eventName, data)
    self:showEffect(data.show);
end

--金钱显示特效
function M:MV_G_JinQ_skill2_1_Model_CreateFootEffect(eventName, data)
